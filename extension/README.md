...
`npm run smoke:extension` launches Microsoft Edge with `extension\dist`, serves mocked ChatGPT, Claude, Tongyi Qianwen, Doubao, and DeepSeek pages through Playwright, verifies that the content script renders each platform rail, node track, round search/settings triggers, and Tampermonkey-style popover cards, and confirms ChatGPT HTML plus all supported batch ZIP exports reach Chrome's downloads API.
...
- an in-page right-side rail with current-conversation export from the settings popover
...
- Manifest V3 update check through the userscript-compatible version marker, badge, auto-check setting, and update dialog
...
- Tampermonkey-style right-side node rail, circular search/settings triggers, settings/search popover cards, header version marker, platform favicon card, switch controls, GitHub project action, and icon action colors
