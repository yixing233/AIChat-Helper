...
(function () {
  ...
  const isDeepSeekPath =
    /^\/(?:$|a\/chat\/s(?:\/[0-9a-f-]{36})?\/?|chat(?:\/[0-9a-f-]{36})?\/?)$/i.test(
      pathname,
    );
  ...
  const SCRIPT_UPDATE_URL =
    "https://raw.githubusercontent.com/yixing233/AIChat-Helper/master/AIChat-Helper.user.js";
  const SCRIPT_DOWNLOAD_URL =
    "https://github.com/yixing233/AIChat-Helper/raw/master/AIChat-Helper.user.js";
  const SCRIPT_CHANGELOG_URL =
    "https://raw.githubusercontent.com/yixing233/AIChat-Helper/master/update.json";
  ...
  const CLAUDE_CONV_PATH_RE =
    /\/api\/organizations\/([0-9a-f-]{36})\/chat_conversations\/([0-9a-f-]{36})/i;
  const CLAUDE_CONV_LIST_PATH_RE =
    /\/api\/organizations\/([0-9a-f-]{36})\/chat_conversations_v2/i;
  ...
  function parseClaudeHeaders(headersLike) {
    ...
    if (typeof headersLike === "object") {
      Object.entries(headersLike).forEach(
        ([k, v]) => (out[String(k).toLowerCase()] = String(v)),
      );
    }
    return out;
  }
  ...
  function parseClaudeApiMeta(rawUrl) {
    try {
      ...
      if (!m) return null;
      return {
        orgId: String(m[1] || "").trim(),
        convId: String(m[2] || "").trim(),
      };
    } catch (_) {
      return null;
    }
  ...
  function installClaudeCaptureHooks() {
    ...
    window.fetch = function (input, init) {
      try {
        ...
        if (
          url &&
          (isClaudeConversationUrl(url) || isClaudeConversationListUrl(url))
        ) {
          claudeCapturedHeaders = sanitizeClaudeHeaders({
            ...parseClaudeHeaders(
              typeof input !== "string" ? input?.headers : null,
            ),
    ...
    XMLHttpRequest.prototype.setRequestHeader = function (name, value) {
      if (this.__aiNodesClaudeHeaders)
        this.__aiNodesClaudeHeaders[String(name).toLowerCase()] = String(value);
      return nativeSetHeader.call(this, name, value);
    };
    XMLHttpRequest.prototype.send = function (body) {
      try {
        const url = this.__aiNodesClaudeUrl
          ? new URL(this.__aiNodesClaudeUrl, window.location.origin).toString()
          : "";
        if (
          url &&
          (isClaudeConversationUrl(url) || isClaudeConversationListUrl(url))
  ...
  function rememberClaudeDownloadedHtmlFile(entry) {
    ...
    if (claudeRecentDownloadedHtmlFiles.length > 20)
      claudeRecentDownloadedHtmlFiles.length = 20;
  }

  function findClaudeDownloadedHtmlFile(widgetCode = "", suggestedName = "") {
    const rawWidget = String(widgetCode || "")
      .trim()
      .toLowerCase();
    const suggest = String(suggestedName || "")
      .trim()
      .toLowerCase();
    const normalizeHtmlName = (value) =>
      String(value || "")
        .trim()
        .toLowerCase()
        .replace(/[?#].*$/g, "")
    ...
    if (isCadHoleWidget) {
      ...
      if (rawWidget.includes("cad异形孔参数提取交互演示"))
        hints.push("cad异形孔参数提取交互演示");
      if (rawWidget.includes("腰形孔") && rawWidget.includes("d形孔"))
        hints.push("腰形孔");
    }
    ...
    for (const item of claudeRecentDownloadedHtmlFiles) {
      ...
      if (suggest && nameLower.includes(suggest) && suggest.length >= 6)
        return item;
      if (suggestNorm && nameNorm) {
        const shortest = Math.min(suggestNorm.length, nameNorm.length);
        if (
          shortest >= 4 &&
          (nameNorm === suggestNorm ||
            nameNorm.includes(suggestNorm) ||
            suggestNorm.includes(nameNorm))
        ) {
          return item;
        }
      }
      if (
        isCadHoleWidget &&
        hints.length >= 2 &&
        hints.every((needle) => contentLower.includes(needle))
      )
        return item;
    }
    return null;
  ...
  function installClaudeDownloadCaptureHooks() {
    ...
    HTMLAnchorElement.prototype.click =
      function AIChatHelperClaudeAnchorClick() {
        try {
          const href = String(
  ...
  function removeClaudePromptSuggestionsFromHtml(html) {
    ...
    try {
      ...
      Array.from(doc.querySelectorAll("script")).forEach((el) => {
        ...
        if (
          /\bsendPrompt\s*=|\bsendPrompt\s*\(/.test(code) &&
          !/\b(addEventListener|querySelector|querySelectorAll|getElementById)\b/.test(
            code,
          )
        ) {
          el.remove();
      ...
      if (/<(?:!doctype\s+html|html|head|body)\b/i.test(raw)) {
        const doctype =
          raw.startsWith("<!doctype") || raw.startsWith("<!DOCTYPE")
            ? "<!doctype html>\n"
            : "";
      ...
      return raw
        .replace(
          /<button\b[^>]*\bonclick=(["'])[^"']*\bsendPrompt\s*\([^"']*\1[^>]*>[\s\S]*?<\/button>/gi,
          "",
        )
        .replace(
          /<a\b[^>]*\bonclick=(["'])[^"']*\bsendPrompt\s*\([^"']*\1[^>]*>[\s\S]*?<\/a>/gi,
          "",
        )
        .trim();
    }
  ...
  function buildClaudeWidgetHtmlDocument(
    widgetCode,
    title = "Claude 交互演示",
  ) {
  ...
  function normalizeClaudeInlineHtmlContent(html, title = "Claude 交互演示") {
    const raw = removeClaudePromptSuggestionsFromHtml(
      String(html || "").trim(),
    );
    if (!raw) return "";
    if (/<(?:!doctype\s+html|html|head|body)\b/i.test(raw)) return raw;
    return buildClaudeWidgetHtmlDocument(
      raw,
      String(title || "Claude 交互演示").replace(/\.html?$/i, ""),
    );
  }
  ...
  function normalizeClaudeWidgetFilePart(
    widgetCode,
    index = 0,
    toolUseId = "",
  ) {
    ...
    try {
      ...
      const doc = parser.parseFromString(
        `<div id="__claude_widget_file_root__">${raw}</div>`,
        "text/html",
      );
    ...
    const captured = canUseCapturedHtml
      ? findClaudeDownloadedHtmlFile(raw, fileName)
      : null;
    if (captured?.fileName)
      fileName = String(captured.fileName).trim() || fileName;
    const inlineContent =
      removeClaudePromptSuggestionsFromHtml(
        String(captured?.content || "").trim(),
  ...
  function parseClaudeWidgetHtmlSource(widgetHtml) {
    ...
    try {
      ...
      const bodyStyle = body
        ? String(body.getAttribute("style") || "").trim()
        : "";
  ...
  function mountClaudeWidgetPreview(el, widgetHtml, title = "") {
    ...
    const { styles, bodyHtml, scripts, bodyClassName, bodyStyle } =
      parseClaudeWidgetHtmlSource(raw);
    const normalizedStyles = normalizeClaudeWidgetStyles(styles);
    const bodyClassAttr = bodyClassName
      ? ` ${escapeHtmlAttr(bodyClassName)}`
      : "";
    const bodyStyleAttr = bodyStyle
      ? ` style="${escapeHtmlAttr(bodyStyle)}"`
      : "";
    shadow.innerHTML = `
            <style>
                :host {
    ...
    const sandboxDocument = createClaudeWidgetSandboxDocument(
      shadow,
      widgetBodyEl,
    );
    const sandboxWindow = {
  ...
  function normalizeClaudeImagePart(part, index = 0) {
    ...
    const fileName = String(
      part.fileName || part.file_name || part.name || "",
    ).trim();
    const previewUrl = resolveClaudeAssetUrl(
      part.previewUrl || part.preview_url || part.url || part.src || "",
    );
    const thumbnailUrl = resolveClaudeAssetUrl(
      part.thumbnailUrl ||
        part.thumbnail_url ||
        part.thumbUrl ||
        part.thumb_url ||
    ...
    return {
      ...
      fileUuid: String(
        part.fileUuid || part.file_uuid || part.uuid || "",
      ).trim(),
      previewUrl,
      thumbnailUrl,
  ...
  function isClaudeExportableSvgElement(svgEl) {
    ...
    if (String(svgEl.getAttribute("role") || "").toLowerCase() === "img")
      return true;
  ...
  function normalizeClaudeSvgPart(svgEl, index = 0) {
    ...
    const firstNonMeta =
      Array.from(clone.childNodes || []).find((node) => {
    ...
    let width = /%/.test(rawWidthAttr)
      ? 0
      : parseClaudeNumericDimension(rawWidthAttr);
    let height = /%/.test(rawHeightAttr)
      ? 0
      : parseClaudeNumericDimension(rawHeightAttr);
    ...
    if (width > 0 && (!rawWidthAttr || /%/.test(rawWidthAttr)))
      clone.setAttribute("width", String(width));
    if (height > 0 && (!rawHeightAttr || /%/.test(rawHeightAttr)))
  ...
  function inferClaudeWidgetShapeKind(html, title = "") {
    ...
    const checks = [
      ...
      },
      {
        kind: "triangle",
        patterns: [
          /activekey\s*[:=]\s*['"`]triangle/,
          /currenthole\s*[:=]\s*['"`]triangle/,
          /三角孔/,
          /\btriangle\b/,
        ],
      },
  ...
  function isClaudeCadHoleWidgetHtml(html, title = "") {
    const raw = `${String(title || "")}\n${String(html || "")}`.toLowerCase();
    if (raw.includes('id="hole-svg"') || raw.includes("id='hole-svg'"))
      return true;
    if (raw.includes('id="hole-title"') || raw.includes("id='hole-title'"))
      return true;
    if (raw.includes("switchhole(")) return true;
    if (raw.includes("cad异形孔") || raw.includes("cad hole parameter"))
      return true;
    const holeLabels = ["腰形孔", "d形孔", "三角孔", "十字孔"];
    const hasAllHoleLabels = holeLabels.every((label) => raw.includes(label));
    const hasInteractiveHoleControls =
      /(?:onclick|oninput|addeventlistener|data-[\w-]+)=/i.test(raw) &&
  ...
  function hasClaudeSvgDrawableContent(svgEl) {
    ...
    const drawables = Array.from(
      scopedRoot.querySelectorAll(
        "path, rect, circle, ellipse, polygon, polyline, line, text, image",
      ),
  ...
  function extractClaudeRenderablePartsFromHtmlFragment(html) {
    ...
    root
      .querySelectorAll('#vis-container svg, svg[role="img"], img')
      .forEach(pushNode);
  ...
  function isClaudeToolBoilerplateText(text) {
    ...
    if (/^\[This tool call rendered an interactive widget/i.test(t))
      return true;
    return false;
  }
  ...
  function extractClaudeVisibleTextFromHtmlFragment(html) {
    ...
    const hasRenderableMedia =
      extractClaudeRenderablePartsFromHtmlFragment(raw).length > 0;
    const root = parseClaudeHtmlNodeRoot(raw);
    ...
    const walk = (node) => {
      ...
      const hasBlockChild = childElements.some((el) =>
        BLOCK_TAGS.has(String(el.tagName || "").toLowerCase()),
      );
  ...
  function summarizeClaudeToolInput(name, input) {
    ...
    const out = [];
    if (
      Array.isArray(payload.loading_messages) &&
      payload.loading_messages.length
    ) {
      out.push(
        `加载提示: ${payload.loading_messages
          .map((x) => String(x || "").trim())
          .filter(Boolean)
          .join(" / ")}`,
      );
  ...
  function looksLikeClaudeWidgetHtml(value) {
    ...
    return (
      ...
      (/id=["']vis-container["']/i.test(raw) ||
        ...
        (/<style[\s\S]*<\/style>/i.test(raw) &&
          /<(?:div|section|article|main|svg|pre)\b/i.test(raw)))
    );
  }
  ...
  function findClaudeWidgetCodeInValue(value, depth = 0, seen = new Set()) {
    ...
    if (typeof value === "string")
      return looksLikeClaudeWidgetHtml(value) ? value : "";
  ...
  function extractClaudePartsFromContent(content) {
    ...
    items.forEach((frag, idx) => {
      ...
      if (fragType === "tool_use") {
        ...
        const widgetCode =
          findClaudeWidgetCodeInValue(frag.input) ||
          findClaudeWidgetCodeInValue(frag);
        const widgetFilePart =
          toolName === "visualize:show_widget"
            ? normalizeClaudeWidgetFilePart(
                widgetCode,
                idx,
                String(frag.id || "").trim(),
              )
        ...
        if (toolName === "visualize:show_widget") {
          ...
          const widgetPreviewSource =
            String(widgetFilePart?.inlineContent || "").trim() ||
            getClaudeWidgetPreviewSource(
              widgetCode,
              String(widgetFilePart?.fileName || "").trim(),
            );
          const widgetText =
          ...
          if (widgetText) {
            out.push({
              ...
              input:
                frag.input && typeof frag.input === "object"
                  ? JSON.parse(JSON.stringify(frag.input))
                  : frag.input,
              text: widgetText,
            });
          }
        ...
        const widgetText =
          toolName === "visualize:show_widget"
            ...
            : "";
        const summary =
          widgetText || summarizeClaudeToolInput(toolName, frag.input);
        out.push({
          type: "tool_use",
          name: toolName,
          toolUseId: String(frag.id || "").trim(),
      ...
      if (fragType === "tool_result") {
        ...
        if (shouldIgnoreClaudeToolName(toolName)) return;
        const widgetCode =
          findClaudeWidgetCodeInValue(frag.content) ||
          findClaudeWidgetCodeInValue(frag);
        if (widgetCode) {
          const widgetFilePart = normalizeClaudeWidgetFilePart(
            widgetCode,
            idx,
            String(frag.tool_use_id || frag.id || "").trim(),
          );
          if (widgetFilePart) out.push(widgetFilePart);
          const widgetPreviewSource =
            String(widgetFilePart?.inlineContent || "").trim() || widgetCode;
          const widgetText =
            extractClaudeVisibleTextFromHtmlFragment(widgetPreviewSource);
          if (widgetText) {
            out.push({
              ...
              content: Array.isArray(frag.content)
                ? JSON.parse(JSON.stringify(frag.content))
                : frag.content,
              text: widgetText,
            });
          }
  ...
  function extractClaudePartsFromFiles(files) {
    ...
    list.forEach((file, idx) => {
      ...
      const fileName =
        String(file.file_name || file.name || file.filename || "").trim() ||
        `文件${idx + 1}`;
      const fileUuid = String(file.file_uuid || file.uuid || "").trim();
      const fileKind = String(file.file_kind || file.kind || "")
        .trim()
        .toLowerCase();
      ...
      if (isHtml) {
        ...
        const inlineContent = normalizeClaudeInlineHtmlContent(
          directHtml || cached?.content || "",
          fileName,
        );
        ...
        const imagePart = normalizeClaudeImagePart(
          {
            ...file,
            previewUrl:
              file.preview_asset?.url || file.preview_url || file.url || "",
            thumbnailUrl: file.thumbnail_asset?.url || file.thumbnail_url || "",
            width:
              file.preview_asset?.image_width ||
              file.thumbnail_asset?.image_width ||
              file.image_width ||
              0,
            height:
              file.preview_asset?.image_height ||
              file.thumbnail_asset?.image_height ||
              file.image_height ||
              0,
            mimeType,
          },
  ...
  async function hydrateClaudeHtmlFileParts(messages) {
    ...
    list.forEach((msg) => {
      ...
      parts.forEach((part) => {
        ...
        if (cached?.content) {
          part.inlineContent = normalizeClaudeInlineHtmlContent(
            cached.content,
            fileName,
          );
          part.inlineExt = "html";
          part.inlineArchiveKey =
            part.inlineArchiveKey ||
            `file_${hashTextForArchiveKey(`${part.fileUuid || fileName}:${part.inlineContent}`)}`;
          return;
        }
    ...
    for (const target of targets) {
      try {
        ...
        if (
          ...
          !looksLikeClaudeWidgetHtml(text) &&
          !/\.html?$/i.test(target.fileName)
        )
          continue;
        const inlineContent = normalizeClaudeInlineHtmlContent(
          text,
        ...
        target.part.inlineArchiveKey =
          target.part.inlineArchiveKey ||
          `file_${hashTextForArchiveKey(`${target.part.fileUuid || target.fileName}:${inlineContent}`)}`;
        rememberClaudeDownloadedHtmlFile({
  ...
  function buildClaudeTextFromParts(parts, mode = "plain") {
    ...
    const renderMarkdownImagePart = (mediaPart, fallbackName = "") => {
      ...
      const mediaType = String(mediaPart.type || "")
        .trim()
        .toLowerCase();
      imageSerial += 1;
      if (mediaType === "svg") {
        const name =
          String(mediaPart.fileName || fallbackName || "").trim() ||
          `图像${imageSerial}`;
      ...
      if (mediaType === "image") {
        ...
        const url = resolveExportAssetUrl(
          mediaPart.previewUrl || mediaPart.url || mediaPart.thumbnailUrl || "",
        );
        return url
    ...
    const renderInlineFileSnapshotForMarkdown = (part, fileName) => {
      ...
      const mediaParts =
        extractClaudeRenderablePartsFromHtmlFragment(inlineContent);
      ...
      if (isClaudeCadHoleWidgetHtml(inlineContent, fileName)) {
        return renderMarkdownImagePart(
          buildClaudeWidgetFallbackSvgPart(inlineContent, fileName),
          fileName,
        );
      }
      const parsed = parseClaudeWidgetHtmlSource(inlineContent);
      const bodyHtml = sanitizeMarkdownWidgetBody(
        parsed.bodyHtml || inlineContent,
      );
      if (!bodyHtml) return "";
      const className = `claude-md-widget-${hashTextForArchiveKey(inlineContent)}`;
      const styleBlock = scopeMarkdownWidgetStyles(
        parsed.styles || "",
        className,
      );
    ...
    list.forEach((part) => {
      ...
      if (type === "file") {
        ...
        const snapshot =
          mode === "markdown"
            ? renderInlineFileSnapshotForMarkdown(part, name)
            : "";
  ...
  function parseClaudeMessagesFromResponse(respJson) {
    const chatMessages = Array.isArray(respJson?.chat_messages)
      ? respJson.chat_messages
      : [];
    const out = [];
    chatMessages.forEach((m, idx) => {
      ...
      out.push({
        ...
        parts,
        sourceMessageId:
          String(m?.uuid || "").trim() || `claude-msg-${idx + 1}`,
  ...
  async function getClaudeMessagesByApi() {
    ...
    const orgId =
      getClaudeOrgIdFromCookie() ||
      parseClaudeApiMeta(claudeCapturedHeaders?.__url || "")?.orgId ||
      "";
    if (!convId || !orgId) return [];

    const u = new URL(
      `https://claude.ai/api/organizations/${encodeURIComponent(orgId)}/chat_conversations/${encodeURIComponent(convId)}`,
    );
  ...
  function queueQwenPendingApiPayload(url, rawText, source) {
    ...
    if (qwenSuppressCapturedPayloads > 0) {
      qwenNodeLog("pending:skip-suppressed", {
        source,
        url,
        suppressDepth: qwenSuppressCapturedPayloads,
      });
      return;
    }
    ...
    try {
      ...
      const payloadSessionId = String(
        payloadUrl.searchParams.get("session_id") || "",
      ).trim();
      ...
      if (
        ...
        qwenNodeLog("pending:skip-stale", {
          source,
          payloadSessionId,
          currentSessionId,
        });
    ...
    qwenNodeLog("pending:push", {
      source,
      url,
      size: qwenPendingApiPayloads.length,
    });
    ...
    window.fetch = function (input, init) {
      try {
        ...
        if (input && typeof input === "object")
          captureChatGPTHeaders(input.headers);
      } catch (e) {
  ...
  function cleanupQwenInternalRequestMarks() {
    ...
    const now = Date.now();
    Array.from(qwenInternalRequestMarks.entries()).forEach(
      ([url, expiresAt]) => {
        if (!expiresAt || expiresAt <= now)
          qwenInternalRequestMarks.delete(url);
      },
    );
  }
      ...
      if (isQwen) {
        ...
        try {
          sessionStorage.removeItem(
            `ai-nodes-qwen-init-unlock-${currentConvId}`,
          );
        } catch (e) {
          // ignore
        }
  ...
  const initialVisibleLimit = Math.max(
    3,
    Math.min(
      12,
  ...
  const CONFIG = {
    ...
    readingLineOffset: Math.max(
      10,
      Math.min(500, getGlobalValue(READING_LINE_KEY, 150)),
    ),
  ...
  if (removeAds && document.body)
    document.body.classList.add("ai-nodes-hide-ads");
  if (hideDeepSeekNativeNav && document.body)
  ...
  function getMaxRailTopWithSettingsSpace() {
    ...
    const reserveBottom =
      SETTINGS_BUTTON_SAFE_HEIGHT +
      SETTINGS_BUTTON_GAP +
      SETTINGS_VIEWPORT_PADDING;
    return Math.max(
      0,
      Math.floor(window.innerHeight - reserveBottom - railRect.height),
    );
  }
  ...
  function ensureNavigatorMounted() {
    ...
    document.body.classList.toggle(
      ...
      Boolean(isDeepSeek && hideDeepSeekNativeNav),
    );
    if (!container.isConnected) {
      document.body.appendChild(container);
      qwenNodeLog("nav:reattach-container", {
  ...
  function showTooltipForDot(dotEl, node) {
    ...
    globalTooltip.innerText =
      String(node.text || "").slice(0, 150) +
      (String(node.text || "").length > 150 ? "..." : "");
    ...
    setTimeout(() => {
      ...
      if (tooltipRect.bottom > winHeight - 10) {
        globalTooltip.style.top =
          winHeight - tooltipRect.height - 10 + tooltipRect.height / 2 + "px";
      }
  ...
  function resolveThemeAccentRgb() {
    ...
    try {
      ...
      for (const selector of probeSelectors) {
        ...
        const rgb =
          parseCssColorToRgb(style.color) ||
          parseCssColorToRgb(style.backgroundColor);
        if (rgb) return rgb;
      }
    } catch (e) {
  ...
  function scheduleJumpToastHideAfterArrived(nodeId) {
    ...
    jumpToastArrivedTimer = setTimeout(() => {
      jumpToastArrivedTimer = 0;
      if (!jumpToastPendingNodeId || String(jumpToastPendingNodeId) !== id)
        return;
  ...
  function removeJumpToastImmediately() {
    hideJumpToast();
    if (jumpToastEl && jumpToastEl.parentNode)
      jumpToastEl.parentNode.removeChild(jumpToastEl);
    if (jumpToastSecondaryEl && jumpToastSecondaryEl.parentNode)
  ...
  function getUpdateLogEntriesBetweenVersions(
    changelog,
    currentVersion,
    latestVersion,
  ) {
    ...
    return versions
      ...
      .sort((a, b) =>
        compareVersionParts(String(b?.version || ""), String(a?.version || "")),
      );
  }

  function showScriptUpdateDialog(
    currentVersion,
    latestVersion,
    changelogEntries = [],
  ) {
  ...
  function setVersionUpdateBadge(latestVersion = "") {
    ...
    const hasUpdate =
      ...
      compareVersionParts(availableUpdateVersion, SCRIPT_VERSION) > 0;
    ...
    try {
      ...
      if (!silent) {
        ...
        try {
          ...
          changelogEntries = getUpdateLogEntriesBetweenVersions(
            changelog,
            currentVersion,
            latest.version,
          );
  ...
  function isJumpTargetArrived(nodeId) {
    ...
    if (
      (isQwen || isDoubao || isDeepSeek) &&
      String(activeNodeId || "") === id
    ) {
      return true;
    }
    ...
    const readingLineOffset = Math.max(
      10,
    ...
    const viewportPad = 4;
    const inViewport =
      rect.bottom >= containerRect.top + viewportPad &&
      rect.top <= containerRect.bottom - viewportPad;
    if ((isQwen || isDoubao || isDeepSeek || isClaude) && inViewport)
      return true;
    ...
    const nearBottomLimit =
      maxScrollTop > 0 && currentScrollTop >= maxScrollTop - 2;
  ...
  function showJumpToast(node) {
    ...
    const nodeIndex =
      Math.max(
        0,
        nodes.findIndex((n) => String(n?.id || "") === String(node?.id || "")),
      ) + 1;
    ...
    if (
      ...
      if (prevStateText || prevNodeText) {
        renderJumpToastSecondary(
          prevStateText,
          prevNodeText,
          /^已达到/.test(prevStateText),
        );
      }
    }
  ...
  function getMessages() {
    ...
    if (isChatGPT) {
      const msgs = document.querySelectorAll(
        '[data-message-author-role], article[data-testid^="conversation-turn-"] [data-message-author-role="user"], [data-testid="conversation-turn-user"]',
      );
      const seen = new Set();
      msgs.forEach((el, index) => {
        ...
        const isUser =
          role === "user" ||
          el.getAttribute("data-testid") === "conversation-turn-user";
        if (isUser) {
          const id =
      ...
      if (
        !qwenVirtualNodesLoaded ||
        qwenVirtualNodesDirty ||
        !qwenVirtualNodesSessionId ||
        (currentSessionId && qwenVirtualNodesSessionId !== currentSessionId)
      ) {
      ...
      if (
        !qwenVirtualNodesSessionId ||
        !currentSessionId ||
        qwenVirtualNodesSessionId === currentSessionId
      ) {
        const cached = Array.isArray(qwenVirtualNodesCache)
          ? qwenVirtualNodesCache
          : [];
        const merged = cached.slice();
        qwenPendingDomNodes.forEach((p) => {
          if (!p || !p.text) return;
          if (hasQwenApiNodeForDom(p.sourceMessageId || "", p.text || ""))
            return;
          merged.push(p);
        });
        merged.sort(
          (a, b) =>
            getQwenSessionIndexValue(a?.sessionIndex) -
            getQwenSessionIndexValue(b?.sessionIndex),
        );
        merged.forEach((item, idx) => {
          ...
          list.push({
            ...item,
            sessionIndex:
              getQwenSessionIndexValue(item.sessionIndex) !== -1
      ...
      const cached = Array.isArray(doubaoVirtualNodesCache)
        ? doubaoVirtualNodesCache
        : [];
      ...
      doubaoPendingDomNodes.forEach((p) => {
        if (!p || !p.text) return;
        if (hasDoubaoApiNodeForDom(p.sourceMessageId || "", p.text || ""))
          return;
        merged.push(p);
      });
      merged.sort((a, b) => {
      ...
      const cached = Array.isArray(deepseekVirtualNodesCache)
        ? deepseekVirtualNodesCache
        : [];
      ...
      const currentConvId = String(
        getClaudeConversationIdFromUrl() || "",
      ).trim();
      if (
        currentConvId &&
        claudeVirtualNodesConversationId &&
      ...
      const cached = Array.isArray(claudeVirtualNodesCache)
        ? claudeVirtualNodesCache
        : [];
  ...
  function getDoubaoDomUserSignature() {
    ...
    for (let i = 0; i < rows.length; i += step) {
      ...
      const txt = String(getDoubaoUserTextFromRow(row) || "")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 32);
      pick.push(`${msgId}|${txt}`);
  ...
  function getDeepSeekSessionIdFromLocation() {
    try {
      ...
      const pathMatch =
        u.pathname.match(/\/a\/chat\/s\/([0-9a-f-]{36})/i) ||
  ...
  function getDeepSeekSessionIdFromLinks() {
    const linkCandidates = Array.from(
      document.querySelectorAll(
        'a[href*="/a/chat/s/"], a[href*="/chat/"], a[href*="chat_session_id="], a[href*="session_id="]',
      ),
    );
    for (const a of linkCandidates) {
      const href = String(a.getAttribute("href") || "");
      const m =
        href.match(/\/a\/chat\/s\/([0-9a-f-]{36})/i) ||
  ...
  function installDeepSeekCaptureHooks() {
    ...
    XMLHttpRequest.prototype.send = function () {
      try {
        if (
          this.__aiNodesDeepSeekUrl &&
          (isDeepSeekHistoryUrl(this.__aiNodesDeepSeekUrl) ||
            isDeepSeekPageListUrl(this.__aiNodesDeepSeekUrl))
        ) {
          const mergedHeaders = sanitizeDeepSeekHeaders(
            this.__aiNodesDeepSeekHeaders || {},
          );
  ...
  function guessDeepSeekRole(item) {
    ...
    if (low.includes("assistant") || low.includes("bot") || low.includes("ai"))
      return "assistant";
    if (
      low.includes("user") ||
      low.includes("human") ||
    ...
    if (item?.from_bot === true || item?.is_assistant === true)
      return "assistant";
    return "assistant";
  }
  ...
  function parseDeepSeekMessagesFromResponse(resp) {
    ...
    const chatMessages = Array.isArray(bizData?.chat_messages)
      ...
      if (fragments.length) {
        const orderedFragments = fragments
          ...
          .sort((a, b) => {
            const typeA =
              String(
                a?.frag?.type || a?.frag?.content_type || "",
              ).toUpperCase() || "UNKNOWN";
            const typeB =
              String(
                b?.frag?.type || b?.frag?.content_type || "",
              ).toUpperCase() || "UNKNOWN";
        ...
        orderedFragments.forEach(({ frag, fragIdx }) => {
          const fragmentType =
            String(frag?.type || frag?.content_type || "").toUpperCase() ||
            "UNKNOWN";
          ...
          if (fragmentType === "RESPONSE") {
            const text = adaptDeepSeekCitationText(
              typeof frag?.content === "string" ? frag.content : "",
              citationMap,
            );
            pushFragmentMessage(
              text,
          ...
          const fallbackText = Array.from(
            new Set(bucket.map((s) => String(s || "").trim()).filter(Boolean)),
          )
            .join("\n\n")
            .trim();
          pushFragmentMessage(
  ...
  function aggregateDeepSeekMessagesForExport(messages) {
    ...
    groups.forEach((group, idx) => {
      if (group.role === "user") {
        const text = Array.from(new Set(group.userParts.filter(Boolean)))
          .join("\n\n")
          .trim();
        if (!text) return;
        out.push({
      ...
      const thoughtText = Array.from(
        new Set(group.thoughtParts.filter(Boolean)),
      )
        .join("\n\n")
        .trim();
      const responseText = Array.from(
        new Set(group.responseParts.filter(Boolean)),
      )
        .join("\n\n")
        .trim();
      const fallbackText = Array.from(
        new Set(group.fallbackParts.filter(Boolean)),
      )
        .join("\n\n")
        .trim();
      if (thoughtText) {
  ...
  function parseDeepSeekTsValue(raw) {
    ...
    if (/^\d+(?:\.\d+)?$/.test(str)) {
      ...
      return {
        value: ms,
        text: Number.isFinite(dt.getTime()) ? dt.toLocaleString() : str,
      };
    }
    const parsed = Date.parse(str);
    if (Number.isFinite(parsed))
      return { value: parsed, text: new Date(parsed).toLocaleString() };
    ...
    return {
      chatSession: chatSession
        ? {
            ...
            updatedAtRaw:
              chatSession.updated_at == null
                ? ""
                : String(chatSession.updated_at),
            seqId: chatSession.seq_id == null ? "" : String(chatSession.seq_id),
            agent: String(chatSession.agent || ""),
            version:
              chatSession.version == null ? "" : String(chatSession.version),
            currentMessageId:
              chatSession.current_message_id == null
                ? ""
                : String(chatSession.current_message_id),
            insertedAt: insertedNorm.text,
            insertedAtValue: insertedNorm.value,
            insertedAtRaw:
              chatSession.inserted_at == null
                ? ""
                : String(chatSession.inserted_at),
          }
        : null,
  ...
  async function getDeepSeekMessagesByApi() {
    ...
    if (!sessionId) {
      console.warn(
        "AI-Chat-Helper: DeepSeek 未找到会话 ID，无法请求 history_messages",
      );
      return [];
    }
    ...
    try {
      ...
      if (!resp.ok) {
        console.warn(
          `AI-Chat-Helper: DeepSeek history_messages 请求失败 (${resp.status})`,
        );
        return [];
      }
  ...
  function getDeepSeekDomRows() {
    const area =
      document.querySelector(".ds-virtual-list-visible-items") || document.body;
  ...
  function scanDeepSeekDomNodes() {
    ...
    rows.forEach((row, idx) => {
      ...
      const bubble =
        ...
        row;
      const text = normalizeDeepSeekTextForMatch(
  ...
  function getDeepSeekNodeGroupRange(node) {
    ...
    const assistants = Array.isArray(node?._dsAssistantRows)
      ? node._dsAssistantRows
      : [];
  ...
  function getDeepSeekNodeAlignY(node, scrollEl) {
    ...
    if (!el || !document.body.contains(el))
      return getDeepSeekReadingY(scrollEl);
  ...
  function findDeepSeekDomElementByNode(node) {
    ...
    const direct =
      list.find((n) => String(n?.id || "") === String(node?.id || "")) || null;
    if (direct?.element && document.body.contains(direct.element))
      return direct.element;

    const candidates = deepseekDomNodesSnapshot.length
      ? deepseekDomNodesSnapshot
      : scanDeepSeekDomNodes();
  ...
  function getDeepSeekLiveDomNodes() {
    const usable = (
      Array.isArray(deepseekDomNodesSnapshot) ? deepseekDomNodesSnapshot : []
  ...
  function getDeepSeekScrollMetrics(scrollEl) {
    ...
    if (isWindowScroll) {
      ...
      const maxTop = Math.max(
        0,
        Number((root?.scrollHeight || 0) - (window.innerHeight || 0)),
      );
      return { top, maxTop };
    }
    ...
    const maxTop = Math.max(
      0,
      Number((scrollEl.scrollHeight || 0) - (scrollEl.clientHeight || 0)),
    );
    return { top, maxTop };
  }

  async function findDeepSeekTargetWithDirectionalScroll(
    targetNode,
    targetApiIndex,
    token = 0,
  ) {
    rebindDeepSeekNodesToDom(nodes, true);
    let target =
      getDeepSeekLiveDomNodes().find(
        (n) => String(n?.nodeId || "") === String(targetNode?.id || ""),
      ) || null;
  ...
  async function alignDeepSeekNodeToReadingLine(targetNode, token = 0) {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      rebindDeepSeekNodesToDom(nodes, true);
      const liveNode =
        nodesMap.get(String(targetNode?.id || "")) || targetNode || null;
      ...
      const moved = Number(
        scrollDeepSeekBy(scrollEl, Math.max(-520, Math.min(520, delta))) || 0,
      );
      ...
      const noMove =
        Math.abs(moved) < 1 ||
        Math.abs(metricsAfter.top - metricsBefore.top) < 1;
      if (noMove) {
  ...
  async function jumpToDeepSeekNodeById(nodeId) {
    ...
    const targetEl = await findDeepSeekTargetWithDirectionalScroll(
      targetNode,
      targetIndex,
      token,
    );
  ...
  function scheduleDeepSeekVirtualNodesRefresh(force = false) {
    ...
    getDeepSeekMessagesByApi()
      .then((apiMsgs) => {
        ...
        const userMsgs = apiMsgs.filter(
          (m) => m && m.role === "user" && m.text,
        );
        if (!userMsgs.length) {
          deepseekVirtualNodesLoaded = true;
          return;
        ...
        const built = userMsgs
          .map((m, idx) => {
            const text = String(m.text || "").trim();
            // 采用聚合后的 ID 来源，确保一轮对话仅有一个节点
            const rawId =
              m.sourceMessageId ||
              (typeof m.id === "string" && m.id.includes("export-")
                ? m.id.split("export-")[1]
                : m.id) ||
              String(idx + 1);
  ...
  function buildDoubaoVirtualNodesFromApi(apiMsgs) {
    ...
    const userMsgs = (Array.isArray(apiMsgs) ? apiMsgs : []).filter(
      (m) => m && m.role === "user" && m.text,
    );
    ...
    userMsgs.forEach((m, idx) => {
      ...
      const id =
        sourceMessageId || `doubao-user-${convId}-${idx + 1}-${fallbackStable}`;
  ...
  function jumpToMessage(el, nodeId) {
    ...
    if (isDeepSeek && nodeId) {
      jumpToDeepSeekNodeById(nodeId)
        .then((ok) => {
          if (!ok) {
            console.warn(
              `AI-Chat-Helper: DeepSeek 跳转未命中目标节点 ${nodeId}`,
            );
            hideJumpToast();
          }
        })
    ...
    const isElementValid = (element, expectedNode) => {
      ...
      if (isQwen) {
        ...
        if (resolvedNode) {
          return (
            String(resolvedNode.id || "") === String(expectedNode?.id || "")
          );
        }
        return false;
      }
    ...
    const executeJump = () => {
      ...
      const jumpBehavior =
        !isClaude && !isQwen && !isDoubao && !isDeepSeek ? "auto" : "smooth";
  ...
  function getNearestScrollableAncestor(el, stopAt) {
    ...
    if (
      stopAt &&
      stopAt instanceof HTMLElement &&
      isScrollableContainerElement(stopAt)
    )
      return stopAt;
    return null;
  }
  ...
  function resolveClaudeMessageContainerFromHeading(headingEl, root) {
    ...
    for (let i = 0; i < 10 && cur && cur !== scope.parentElement; i += 1) {
      ...
      if (
        ...
        score += Math.max(
          0,
          1600 - Math.abs(width - Math.min(980, window.innerWidth * 0.72)),
        );
        score += Math.max(0, 900 - height);
        if (
          /max-w-|mx-auto|group|contents|message/i.test(
    ...
    return (
      best ||
      headingEl.closest("div")?.parentElement ||
  ...
  function getClaudeUserMessageBlocks() {
    ...
    const direct = Array.from(
      root.querySelectorAll('[data-user-message-bubble="true"]'),
    )
      .map((bubble) =>
        bubble.closest(
          ".mb-1.mt-6.group, article, section, [data-message-id], div",
        ),
      )
    ...
    direct.forEach((el) => {
      ...
      if (
        el.closest(
          "#ai-nav-panel, #ai-nav-orbital, #ai-nav-export-modal, #ai-nav-batch-export-modal",
        )
      )
        return;
  ...
  function getClaudeNodeElementMatchScore(node, element) {
    ...
    const candidate =
      extractClaudeBlockUserText(element) ||
      normalizeClaudeMatchText(element.innerText || "");
    ...
    if (target.slice(0, 48) && candidate.includes(target.slice(0, 48)))
      score += 40;
  ...
  function findClaudeBlockBySourceMessageId(sourceMessageId) {
    ...
    const all = root.querySelectorAll(
      "[id],[data-message-id],[data-testid],[data-message-uuid],[data-node-id]",
    );
    for (const el of all) {
      ...
      const h = el.matches('h2,[role="heading"]')
        ? el
        : el.querySelector('h2,[role="heading"]');
      const block =
        resolveClaudeMessageContainerFromHeading(h, root) ||
  ...
  function findClaudeMessageElementByNode(node, options = {}) {
    ...
    if (!root) return null;
    const excludeEls =
      options.excludeEls instanceof Set ? options.excludeEls : null;
    const bySourceId = findClaudeBlockBySourceMessageId(
      node?.sourceMessageId || node?.id || "",
    );
    if (
      bySourceId &&
      (!excludeEls || !excludeEls.has(bySourceId)) &&
      isClaudeElementMatchUsable(node, bySourceId, 20)
    )
      return bySourceId;
    const target = normalizeClaudeMatchText(node.text || "");
  ...
  function findClaudeDomElementForNode(node, options = {}) {
    ...
    for (const el of candidates) {
      ...
      const nodeMsgId = String(
        node?.sourceMessageId || node?.id || "",
      ).toLowerCase();
      if (nodeMsgId) {
        const attrs =
          `${el.getAttribute("id") || ""} ${el.getAttribute("data-message-id") || ""} ${el.getAttribute("data-testid") || ""}`.toLowerCase();
        if (attrs.includes(nodeMsgId)) score += 300;
      }
      if (
        el.getAttribute("data-testid") &&
        /message/i.test(el.getAttribute("data-testid"))
      )
        score += 1;
      if (score > bestScore) {
        bestScore = score;
  ...
  function resolveClaudeElementForNode(node, options = {}) {
    ...
    if (
      ...
      cached.isConnected &&
      (!excludeEls || !excludeEls.has(cached)) &&
      isClaudeElementMatchUsable(node, cached, 20)
    ) {
      return cached;
    }
    const resolved =
      findClaudeMessageElementByNode(node, { excludeEls }) ||
      findClaudeDomElementForNode(node, { excludeEls });
  ...
  function getClaudePrimaryScrollContainer() {
    ...
    if (Array.isArray(nodes) && nodes.length) {
      const orderedNodes = nodes.slice().sort((a, b) => {
        const ia = Number.isInteger(Number(a?.sessionIndex))
          ? Number(a.sessionIndex)
          : Number.MAX_SAFE_INTEGER;
        const ib = Number.isInteger(Number(b?.sessionIndex))
    ...
    const candidates = Array.from(
      root.querySelectorAll("div, section, article, main"),
    )
  ...
  function getClaudeActiveNodeByConversationState(scrollEl) {
    ...
    const maxScroll = Math.max(
      0,
      (workingScrollEl.scrollHeight || 0) - (workingScrollEl.clientHeight || 0),
    );
    const currentTop =
      workingScrollEl === window ||
      workingScrollEl === document.documentElement ||
    ...
    if (maxScroll > 20 && maxScroll - currentTop < 40) {
      ...
      if (
        last?.node &&
        last.el &&
        (!last.node.element || !last.node.element.isConnected)
      )
        last.node.element = last.el;
    ...
    if (
      bestEntry.node &&
      bestEntry.el &&
      (!bestEntry.node.element || !bestEntry.node.element.isConnected)
    ) {
  ...
  async function jumpToClaudeNodeById(nodeId) {
    ...
    const targetNode =
      ...
      nodes.find((n) => String(n?.id || "") === targetId) ||
      null;
    let targetIndex = Number.isInteger(Number(targetNode?.sessionIndex))
      ? Number(targetNode.sessionIndex)
      : -1;
    if (targetIndex < 0) {
    ...
    const resolveTargetElement = () => {
      if (targetNode) {
        const strict =
          findClaudeMessageElementByNode(targetNode) ||
          findClaudeDomElementForNode(targetNode);
        if (strict) return strict;
      }
    ...
    const alignToReadingLine = async (el) => {
      const sc =
        getClaudeScrollContainerForElement(el) ||
        getClaudePrimaryScrollContainer();
      for (let i = 0; i < 6; i += 1) {
    ...
    if (!targetEl) {
      const activeNode = activeNodeId
        ? nodesMap.get(String(activeNodeId)) ||
          nodes.find((n) => String(n?.id || "") === String(activeNodeId)) ||
          null
        : null;
      let activeIdx = Number.isInteger(Number(activeNode?.sessionIndex))
        ? Number(activeNode.sessionIndex)
        : -1;
      if (activeIdx < 0)
        activeIdx = Math.max(
          0,
    ...
    if (!targetEl) {
      console.warn("[AI-Chat-Helper][ClaudeJump] no-target-element", {
        nodeId: targetId,
        targetIndex,
      });
      return false;
    ...
    if (token !== claudeNavJumpSeq) return false;
    const sc =
      getClaudeScrollContainerForElement(targetEl) ||
      getClaudePrimaryScrollContainer();
  ...
  function sanitizeQwenQuestionText(text, fileNames = []) {
    ...
    cleaned = cleaned
      ...
      .replace(/\b(查看|预览|删除|编辑|复制|上传|下载)\b/gi, " ");
  ...
  function extractQwenPromptTextByWalker(root, fileNames = []) {
    ...
    while (node) {
      ...
      if (parent && !parent.closest(skipSelector)) {
        const cleaned = sanitizeQwenQuestionText(
          node.nodeValue || "",
          fileNames,
        );
  ...
  function getQwenPromptTextFromRow(row, fileNames = []) {
    ...
    return sanitizeQwenQuestionText(
  ...
  function getQwenQuestionTextFromRow(row) {
    ...
    const attachLines = fileNames.map(
      (name, idx) => `[附件${fileNames.length > 1 ? idx + 1 : ""}] ${name}`,
    );
  ...
  function getQwenUserDomCandidates() {
    const rawElements = getQwenConversationRows().filter(
      (row) => getQwenRowType(row) === "question",
    );
    const seen = new Set();
    const out = [];
    ...
    rawElements.forEach((row, questionIndex) => {
      const bubble =
        row.querySelector('[class*="contentBox"] [class*="bubble"]') ||
        row.querySelector('[class*="bubble"]') ||
        row.querySelector('[class*="contentBox"]') ||
        row;
      const text =
        getQwenQuestionTextFromRow(row) ||
        normalizeQwenTextForMatch(bubble.innerText || "");
      if (!text) return;
  ...
  function reconcileQwenPendingDomNodesWithApi() {
    if (!Array.isArray(qwenPendingDomNodes) || !qwenPendingDomNodes.length)
      return;
    const next = [];
    const seen = new Set();
  ...
  function hasQwenCompletedAnswerForQuestion(baseId) {
    ...
    const answerRows = rows.filter(
      (row) => getQwenRowType(row) === "answer" && getQwenRowId(row) === baseId,
    );
  ...
  async function runQwenAutoApiRefreshFromDom() {
    ...
    const ready = pending.some((n) =>
      hasQwenCompletedAnswerForQuestion(
        normalizeQwenMessageId(n.sourceMessageId || ""),
      ),
    );
    if (!ready) return;
  ...
  function getQwenRowType(row) {
    ...
    const msgIdRaw = String(
      row.getAttribute("data-msgid") || row.getAttribute("data-msg-id") || "",
    ).toLowerCase();
    ...
    // 结构特征辅助判断
    if (row.querySelector('[class*="user"], [class*="human"], [class*="User"]'))
      return "question";
    if (
      row.querySelector('[class*="assistant"], [class*="bot"], [class*="Ai"]')
    )
      return "answer";
  ...
  function getQwenRowFromElement(element) {
    ...
    return element.closest(
      '[class*="questionItem"], [class*="answerItem"], [class*="question-item"], [class*="answer-item"]',
    );
  }
  ...
  function findQwenNodeBySignature(id, text, sessionIndex = -1) {
    ...
    if (normalizedId) {
      const bySource = nodes.find((n) => {
        const nid = normalizeQwenMessageId(
          String(n?.sourceMessageId || n?.id || ""),
        );
        if (nid !== normalizedId) return false;
        if (normalizedSessionIndex === -1) return true;
        return (
          getQwenSessionIndexValue(n?.sessionIndex) === normalizedSessionIndex
        );
      });
    ...
    nodes.forEach((n) => {
      ...
      if (
        normalizedId &&
        normalizeQwenMessageId(String(n?.sourceMessageId || n?.id || "")) ===
          normalizedId
      )
        score += 18;
      ...
      if (
        normalized &&
        t &&
        normalized.includes(t.slice(0, Math.min(24, t.length)))
      )
        score += 3;
  ...
  function resolveQwenNodeFromRow(row, rows) {
    ...
    const questionRows = Array.isArray(rows)
      ? rows.filter((item) => getQwenRowType(item) === "question")
      : [];
    const canUseGlobalSessionIndex =
      questionRows.length > 0 && questionRows.length === nodes.length;
  ...
  function parseDoubaoInlinePayloadSafe(contentValue, depth = 0) {
    ...
    if (Array.isArray(parsed)) {
      ...
      const walk = (node) => {
        ...
        if (typeof node !== "object") return;
        const t =
          typeof node?.content?.text_block?.text === "string"
  ...
  function scheduleClaudeVirtualNodesRefresh(force = false) {
    ...
    if (
      !force &&
      claudeVirtualNodesLoaded &&
      now - claudeVirtualNodesLastFetchAt < 2500
    )
      return;
    claudeVirtualNodesLoading = true;
    getClaudeMessagesByApi()
      .then((apiMsgs) => {
        const liveConvId = String(
          getClaudeConversationIdFromUrl() || "",
        ).trim();
        ...
        (Array.isArray(apiMsgs) ? apiMsgs : []).forEach((m, idx) => {
          ...
          if (!txt) return;
          const sid =
            String(m.sourceMessageId || m.id || "").trim() ||
            `claude-user-${idx + 1}`;
          built.push({
            id: sid,
            sourceMessageId: sid,
        ...
        claudeVirtualNodesConversationId =
          liveConvId || scheduledConvId || claudeVirtualNodesConversationId;
      })
      .catch((e) => {
  ...
  function getDoubaoUserTextFromRow(row) {
    ...
    const normalizeLines = (raw) =>
      String(raw || "")
        ...
        .filter(Boolean)
        .filter((line) => {
          if (/^(查看|复制|重试|分享|进入\s*AI\s*阅读)$/i.test(line))
            return false;
    ...
    const fullText = sanitizeDoubaoUserNodeText(
      row.querySelector('[data-testid="message_content"]')?.innerText || "",
    );
    const plainText = sanitizeDoubaoUserNodeText(
    ...
    const messageBlockText = pickBestLine(
      row.querySelector("[data-message-id]")?.innerText || row.innerText || "",
    );
  ...
  function getDoubaoRowType(row) {
    ...
    if (row?.querySelector?.('[data-plugin-identifier*="receive"]'))
      return "assistant";
  ...
  async function runDoubaoAutoApiRefreshFromDom() {
    ...
    if (!pending.length) return;
    const ready = pending.some((n) =>
      hasDoubaoCompletedAnswerForQuestion(
        normalizeDoubaoMessageId(n.sourceMessageId || ""),
      ),
    );
    if (!ready) return;
  ...
  function findDoubaoNodeBySignature(id, text) {
    ...
    const middle = normalizedText.slice(
  ...
  function resolveDoubaoNodeFromRow(row, rows) {
    ...
    const resolved = findDoubaoNodeBySignature(
      getDoubaoRowId(row),
      getDoubaoRowText(row),
    );
  ...
  function getDoubaoActiveNodeByConversationState(scrollEl) {
    ...
    const { viewportTop, viewportBottom, readingAnchor } =
      getDoubaoViewportMetrics(scrollEl);
  ...
  function getDoubaoNodeIndexByIdOrText(id, text) {
    ...
    nodes.forEach((n, idx) => {
      ...
      if (
        prefix &&
        (t.includes(prefix) ||
          targetText.includes(t.slice(0, Math.min(24, t.length))))
      )
        score += 6;
      if (score > bestScore) {
        bestScore = score;
  ...
  function getDoubaoDomProgressSignature(scrollEl) {
    ...
    const lastId = list.length
      ? String(list[list.length - 1]?.rowId || "")
      : "";
    const top =
      scrollEl && scrollEl !== window && scrollEl !== document.documentElement
        ? Number(scrollEl.scrollTop || 0)
  ...
  function scrollDoubaoBy(scrollEl, delta) {
    if (
      ...
      scrollEl !== window &&
      scrollEl !== document.documentElement &&
      typeof scrollEl.scrollBy === "function"
    ) {
  ...
  async function findDoubaoTargetWithDirectionalScroll(
    apiNode,
    targetApiIndex,
    token = 0,
  ) {
    ...
    while (Date.now() - startedAt < maxDurationMs) {
      ...
      const targetOlderThanDomTop =
        bounds.min !== -1 && targetApiIndex < bounds.min;
      const targetNewerThanDomBottom =
        bounds.max !== -1 && targetApiIndex > bounds.max;
        ...
        if (staleCount >= 1) {
          ...
          if (changed) {
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, 260 + Math.min(1200, noProgressRounds * 140)),
            );
          }
        }
        continue;
      ...
      if (!goingDown) {
        ...
        const stillAboveTop =
          freshBounds.min !== -1 && targetApiIndex < freshBounds.min;
  ...
  async function jumpToDoubaoNodeById(nodeId) {
    ...
    const targetEl = await findDoubaoTargetWithDirectionalScroll(
      targetNode,
      targetIndex,
      token,
    );
  ...
  function findQwenDomElementByNode(node) {
    ...
    if (targetId) {
      const byId = candidates.find(
        (c) => normalizeQwenMessageId(c.id) === targetId,
      );
      if (byId?.element) return byId.element;
    ...
    candidates.forEach((c) => {
      ...
      let score = 0;
      if (msgIdRaw && normalizeQwenMessageId(msgIdRaw) === targetId)
        score += 16;
      if (txt === targetText) score += 14;
      if (targetPrefix && txt.includes(targetPrefix)) score += 8;
      if (targetMiddle && txt.includes(targetMiddle)) score += 4;
  ...
  function getQwenNodeIndexByIdOrText(id, text) {
    ...
    if (targetId) {
      const byId = nodes.findIndex(
        (n) => normalizeQwenMessageId(n?.id || "") === targetId,
      );
      if (byId !== -1) return byId;
      const bySourceId = nodes.findIndex(
        (n) => normalizeQwenMessageId(n?.sourceMessageId || "") === targetId,
      );
      if (bySourceId !== -1) return bySourceId;
    }
  ...
  function computeQwenActiveApiIndexFromViewport() {
    ...
    if (activeNode?.id) {
      const idx = nodes.findIndex(
        (n) => String(n?.id || "") === String(activeNode.id),
      );
      if (idx >= 0) return idx;
    }
    if (activeNodeId) {
  ...
  async function triggerQwenOlderHistoryLoad(
    scrollEl,
    step,
    beforeSig = "",
    token = 0,
  ...
  async function findQwenTargetWithDirectionalScroll(
    targetNode,
    targetApiIndex,
    token = 0,
  ) {
    ...
    while (Date.now() - startedAt < maxDurationMs) {
      ...
      const activeDistance =
        liveIdx !== -1
          ? Math.abs(liveIdx - targetApiIndex)
      ...
      if (!goingDown && targetOlderThanDomTop) {
        if (staleCount >= 1) {
          const changed = await triggerQwenOlderHistoryLoad(
            scrollEl,
            step,
            sigNow,
            token,
          ...
          if (changed) {
            ...
            noProgressRounds += 1;
            await new Promise((resolve) =>
              setTimeout(resolve, 260 + Math.min(1400, noProgressRounds * 140)),
            );
          }
        }
        continue;
      ...
      if (!goingDown) {
        const finalBounds = getQwenDomBoundsByApiOrder();
        const stillAboveTop =
          finalBounds.min !== -1 && targetApiIndex < finalBounds.min;
        if (stillAboveTop) continue;
      }
  ...
  async function jumpToQwenNodeById(nodeId) {
    ...
    const targetNode =
      targetIndex >= 0
        ...
        : nodesMap.get(String(nodeId)) || null;
    if (!targetNode) return false;
    const el = await findQwenTargetWithDirectionalScroll(
      targetNode,
      targetIndex,
      token,
    );
  ...
  function scheduleQwenVirtualNodesRefresh(force = false) {
    ...
    qwenNodeLog("refresh:start", {
      force,
      now,
      cacheSize: qwenVirtualNodesCache.length,
    });

    getQwenMessagesByApi()
      .then((apiMsgs) => {
        if (refreshStorageKey !== storageKey) return;
        if (!Array.isArray(apiMsgs) || !apiMsgs.length) {
          qwenNodeLog("refresh:empty-api", {
            apiCount: Array.isArray(apiMsgs) ? apiMsgs.length : -1,
          });
      ...
      .finally(() => {
        if (refreshStorageKey !== storageKey) return;
        qwenVirtualNodesLoading = false;
        qwenNodeLog("refresh:done", {
          loading: qwenVirtualNodesLoading,
          cacheSize: qwenVirtualNodesCache.length,
        });
      });
  }

  function applyQwenApiMessagesToCache(
    apiMsgs,
    source = "unknown",
    sourceSessionId = "",
  ) {
    ...
    if (
      ...
      currentSessionId &&
      normalizedSourceSessionId !== currentSessionId
    ) {
    ...
    const built = userMsgs
      .map((m, idx) => {
        const id = normalizeQwenMessageId(
          String(m.id || `qwen-user-${idx + 1}`),
        );
        const sourceMessageId = normalizeQwenMessageId(
          String(m?.sourceMessageId || m?.id || id || ""),
        );
        const text = String(m.text || "").trim();
        return {
          id,
          ...
          text,
          sessionIndex:
            getQwenSessionIndexValue(m.sessionIndex) !== -1
              ? getQwenSessionIndexValue(m.sessionIndex)
              : idx,
          element: null,
        };
      })
  ...
  function flushQwenPendingApiPayloads() {
    ...
    return applyQwenApiMessagesToCache(
      parsed,
      "pending-live",
      currentSessionId,
    );
  ...
  function startNodeSearch(targetId) {
    ...
    const searchDirection =
      targetIdx !== -1 && activeIdx !== -1 && targetIdx > activeIdx
        ? "down"
        : "up";
    ...
    if (isQwen && searchDirection === "up") {
      ...
      if (
        preScrollEl &&
        preScrollEl !== window &&
        typeof preScrollEl.scrollBy === "function"
      ) {
    ...
    function doSearchTick() {
      ...
      if (
        isQwen &&
        targetNode &&
        (!targetNode.element || !document.body.contains(targetNode.element))
      ) {
      ...
      const found =
        ...
        (!isQwen ||
          (isQwen &&
            isQwenElementMatchNode(targetNode.element, targetNode) &&
            isDirectionMatched(targetNode.element)));
      ...
      if (attempts > MAX_ATTEMPTS) {
        stopSearch();
        console.warn(
          ` AI-Chat-Helper: ✗ 搜寻超时 (${MAX_ATTEMPTS} 次)，未找到节点 ${targetId}`,
        );
        hideJumpToast();
        return;
      }
      ...
      if (attempts === 1 || attempts % 6 === 0) {
        console.log(
          ` AI-Chat-Helper: 搜寻中 (${attempts}/${MAX_ATTEMPTS}, dir=${searchDirection})...`,
        );
      }
  ...
  function getApproxLoadedMessageCount() {
    ...
    if (isDeepSeek) {
      return document.querySelectorAll(
        '[class*="message"], [data-message-id], article',
      ).length;
    }
  ...
  function maybeRunQwenInitialScrollUnlock(retry = 0) {
    ...
    const qCount = document.querySelectorAll(
      '[class*="questionItem"], [class*="question-item"]',
    ).length;
    const aCount = document.querySelectorAll(
      '[class*="answerItem"], [class*="answer-item"]',
    ).length;
  ...
  function startLoadAllHistory(triggerBtn = null) {
    return new Promise((resolve) => {
      ...
      toast.innerHTML = `<div style="width:18px;height:18px;flex:none;border-radius:999px;background:conic-gradient(from 0deg,${rgbToRgbaText(toastThemeBlueRgb, 0.2)} 0deg,${rgbToRgbaText(toastThemeBlueRgb, 0.95)} 320deg,${rgbToRgbaText(toastThemeBlueRgb, 0.2)} 360deg);mask:radial-gradient(farthest-side,transparent calc(100% - 2px),#000 calc(100% - 1px));-webkit-mask:radial-gradient(farthest-side,transparent calc(100% - 2px),#000 calc(100% - 1px));animation:ai-nodes-spin .9s linear infinite;"></div><div style="min-width:340px;"><div id="load-all-toast-msg" style="color:rgba(0,0,0,0.9);">准备导出：正在回溯历史记录...</div><div style="margin-top:8px;height:6px;background:${rgbToRgbaText(toastThemeBlueRgb, 0.14)};border-radius:99px;overflow:hidden;"><div id="load-all-toast-bar" style="height:100%;width:0%;background:${rgbToRgbaText(toastThemeBlueRgb, 0.86)};transition:width .25s ease;"></div></div><div id="load-all-toast-sub" style="margin-top:6px;font-size:12px;color:rgba(0,0,0,0.85);">轮次: 0 / ${MAX_TICKS} · 已识别消息: 0 · 到顶确认: 0/${STABLE_ZERO_TARGET}</div></div>`;
      toast.appendChild(
      ...
      const intervalId = setInterval(() => {
        ...
        const consumedProgress = Math.min(
          96,
          Math.max(0, Math.round(consumedRatio * 100)),
        );
        ...
        if (currentScrollTop === 0 && lastScrollTop === 0) {
          stableZeroCount++;
          if (stableZeroCount >= STABLE_ZERO_TARGET) {
            finish();
            return;
          }
        } else {
  ...
  function render() {
    ...
    const dotEdgePad = Math.max(
      14,
      Math.ceil(CONFIG.dotSize / 2) + activeGlowPad,
    );
    ...
    const visibleHeight =
      (displayCount - 1) * CONFIG.dotGap + CONFIG.dotSize + dotEdgePad * 2;

    const minH = 100;
    const maxVisibleHeight = Math.floor(
    ...
    const finalVisibleHeight = Math.max(
      minH,
    ...
    const fitsWithoutScroll =
      totalDotsHeight <= finalVisibleHeight - dotEdgePad * 2 - CONFIG.dotSize;
    ...
    const singleNodeMinTrackHeight = Math.max(
      56,
      Math.round(CONFIG.dotSize * 4.8),
    );
    const fullTrackHeight = Math.max(
      minTrackHeight,
      finalVisibleHeight - dotEdgePad * 2,
    );
    const visibleNodeSpan = Math.max(
      0,
  ...
  function update() {
    ...
    if (isQwen || isDeepSeek || isDoubao) {
      const virtualCacheLength = isQwen
        ...
        : isDeepSeek
          ? deepseekVirtualNodesCache.length
          : doubaoVirtualNodesCache.length;
      ...
      const changed = isQwen
        ...
        : isDeepSeek
          ? syncDeepSeekNodesFromApi(currentBatch)
          : syncDoubaoNodesFromApi(currentBatch);
      if (changed) {
        const cacheData = nodes.map((n) => ({
          id: n.id,
          text: n.text,
  ...
  function syncDeepSeekNodesFromApi(currentBatch) {
    ...
    if (hasAnyChange) {
      ...
      currentBatch.forEach((msg) => {
        ...
        node._dsAssistantRows = Array.isArray(msg._dsAssistantRows)
          ? msg._dsAssistantRows
          : [];
      });
    }
  ...
  function syncQwenNodesFromApi(currentBatch) {
    ...
    const qRows = getQwenConversationRows().filter(
      (row) => getQwenRowType(row) === "question",
    );
    ...
    normalizedBatch.forEach((msg, idx) => {
      ...
      if (
        rowNormalized.length > msgText.length &&
        rowNormalized.includes(msgText.slice(0, Math.min(24, msgText.length)))
      ) {
        msg.text = rowText;
      }
    });
    ...
    normalizedBatch.forEach((msg) => {
      ...
      const merged = {
        ...
        ...msg,
        id,
        sourceMessageId: normalizeQwenMessageId(
          String(msg?.sourceMessageId || prev?.sourceMessageId || id),
        ),
        role: "user",
        text,
      ...
      if (
        getQwenSessionIndexValue(prev.sessionIndex) !==
        getQwenSessionIndexValue(merged.sessionIndex)
      ) {
        hasAnyChange = true;
      }
    });
  ...
  function bindComposerRealtimeTriggers() {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        const text = String(
          sendButton.innerText || sendButton.textContent || "",
        ).trim();
        const label = String(
  ...
  function updateActiveNodeOnScroll(options = {}) {
    ...
    if (isQwen || isDeepSeek || isDoubao || isClaude) {
      const node = isQwen
        ...
        : isDeepSeek
          ...
          : isDoubao
            ? getDoubaoActiveNodeByConversationState(scrollEl)
            : getClaudeActiveNodeByConversationState(scrollEl);
      ...
      if (resolvedNode && resolvedNode.id !== activeNodeId) {
        ...
        if (
          (shouldCenterForDoubao || shouldCenterForOthers) &&
          idx >= 0 &&
    ...
    if (isAtBottom) {
      ...
      if (lastNode && lastNode.id !== activeNodeId) {
        ...
        if (Date.now() - orbitalLastInteractionAt > 420)
  ...
  function injectSettings() {
    ...
    if (isQwen) {
      try {
        ...
        console.warn(
          "AI-Chat-Helper: 千问抓包钩子初始化失败，不影响设置按钮注入",
          e,
        );
      }
    }
    ...
    btn.type = "button";
    btn.className =
      "ai-nodes-settings-btn" +
      (isDeepSeek
        ? " ds-icon-button ds-icon-button--l ds-icon-button--s ds-icon-button--sizing-container"
        : "");
    ...
    const findChatGPTHeaderAnchor = () =>
      ...
      document.querySelector(
        "#page-header > div.flex.items-center.justify-center.gap-3.overflow-x-hidden > div.flex.items-center.justify-end.overflow-x-hidden",
      ) ||
    ...
    const applyFallbackHostPlacement = () => {
      if (isChatGPT) {
        ...
        if (anchor) {
          ...
          const top = Math.max(
            8,
            Math.round(rect.top + Math.max(0, (rect.height - 32) / 2) - 1),
          );
          const left = Math.round(
            Math.max(8, Math.min(window.innerWidth - 40, rect.left - 34)),
          );
    ...
    const searchInput = searchControlsPopup.querySelector(
      "#ai-nodes-search-input",
    );
    const searchConfirmBtn = searchControlsPopup.querySelector(
    ...
    const decideSearchResultsVertical = () => {
      ...
      const estimatedMaxHeight = Math.max(
        120,
        Math.round(Math.min(window.innerHeight * 0.44, 380)),
      );
      const aboveTop = controlsRect.top - estimatedMaxHeight;
      const canShowBelow =
        belowTop + estimatedMaxHeight <= window.innerHeight - margin;
    ...
    const placeAndShowSearchResultsPopup = (vertical = "below") => {
      ...
      const left = Math.max(
        margin,
        Math.min(controlsRect.left, window.innerWidth - popupWidth - margin),
      );
      const belowTop = controlsRect.bottom + gap;
    ...
    const hideSearchPopup = () => {
      ...
      const resultVertical =
        searchResultsPopup.dataset.verticalAlign || "below";
      const resultsHiddenTransform =
        resultVertical === "above"
          ? "translateY(8px) scale(0.98)"
          : "translateY(-8px) scale(0.98)";
    ...
    popup.addEventListener("pointerdown", (e) => {
      ...
      const clickedReadingLineTrigger = !!target.closest(
    ...
    const placeSideMenuByTrigger = (menuEl, triggerEl, fallbackWidth = 186) => {
      ...
      const openDirection = canUseLeft
        ? "left"
        : canUseRight
          ? "right"
      ...
      const top = Math.max(
        margin,
        Math.min(desiredTop, window.innerHeight - menuHeight - margin),
      );
      const hiddenTransform =
        openDirection === "left"
          ? "translateX(8px) scale(0.96)"
    ...
    const visibleLimitInput = nodeSettingsMenu.querySelector(
      "#ai-nodes-visible-limit-val",
    );
    const nodeSettingsTrigger = popup.querySelector(
      "#ai-nodes-node-settings-trigger",
    );
    const nodeSettingsTriggerVal = popup.querySelector(
    ...
    if (versionTrigger)
      versionTrigger.addEventListener("click", handleVersionCheckClick);
    ...
    const rlDisplay = readingLineMenu.querySelector(
      "#ai-nodes-reading-line-display",
    );
    const rlTrigger = popup.querySelector("#ai-nodes-reading-line-trigger");
    const rlTriggerVal = popup.querySelector(
      "#ai-nodes-reading-line-trigger-val",
    );
    ...
    if (isQwen) {
      popup
        .querySelector("#ai-nodes-opt-collapse")
    ...
    if (isQwen) {
      popup
        .querySelector("#ai-nodes-opt-ads")
        .addEventListener("change", (e) => {
    ...
    if (isDeepSeek) {
      const hideNavOpt = popup.querySelector(
        "#ai-nodes-opt-hide-deepseek-native-nav",
      );
      if (hideNavOpt) {
    ...
    popup.querySelector("#ai-nodes-export-trigger").onclick = (e) => {
      ...
      const menuWidth = Math.max(
        160,
        Math.round(exportMenu.offsetWidth || 180),
      );
      const menuHeight = Math.max(
        80,
        Math.round(exportMenu.offsetHeight || 104),
      );
      ...
      const rawLeft = canUseRightAlign
      ...
      const top = canShowBelow
        ...
        : Math.max(
            margin,
            Math.min(aboveTop, window.innerHeight - menuHeight - margin),
          );
    ...
    async function getChatGPTAccessToken() {
      ...
      try {
        const sessionResp = await chatGPTPageFetch(
    ...
    function getChatGPTDeviceId() {
      ...
      try {
        const cookieMatch = String(document.cookie || "").match(
          /(?:^|;\s*)oai-did=([^;]+)/i,
        );
    ...
    function detectChatGPTWorkspaceIds() {
      ...
      try {
        const wsRegex =
          /\bws-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\b/i;
      ...
      try {
        ...
        return (Array.isArray(json?.items) ? json.items : [])
          .map((item) => ({
            ...
            title: String(
              item?.gizmo?.display?.name || item?.gizmo?.display_name || "",
            ).trim(),
          }))
          .filter((item) => item.id);
      } catch (e) {
        console.warn(
          "AI-Chat-Helper: 获取 ChatGPT 项目列表失败",
          workspaceId,
          e,
        );
        return [];
    ...
    function extractChatGPTConversationListItems(rawItems, extra = {}) {
      ...
      return items
        .map((item, idx) => {
          const id = String(
            item?.id || item?.conversation_id || item?.conversationId || "",
          ).trim();
          if (!id) return null;
          ...
          const title =
            String(
              item?.title || item?.name || item?.conversation_title || "",
            ).trim() || `会话 ${idx + 1}`;
    ...
    async function fetchChatGPTRootConversations(
      ...
      while (hasMore && out.length < limit && page < maxPages) {
        ...
        const json = await resp.json();
        const pageItems = extractChatGPTConversationListItems(json?.items, {
          workspaceId,
          archived,
        });
        out.push(...pageItems);
        ...
        if (hasMore && out.length < limit) {
          await new Promise((resolve) =>
            setTimeout(resolve, 180 + Math.floor(Math.random() * 120)),
          );
        }
      }
    ...
    async function fetchChatGPTProjectConversations(
      ...
      while (cursor != null && out.length < limit && guard < 20) {
        const resp = await fetch(
          `/backend-api/gizmos/${project.id}/conversations?cursor=${encodeURIComponent(cursor)}`,
          {
            headers: buildChatGPTApiHeaders(token, workspaceId),
          },
        );
        ...
        const json = await resp.json();
        out.push(
          ...extractChatGPTConversationListItems(json?.items, {
            workspaceId,
    ...
    async function fetchChatGPTRecentConversations(limit = 100, maxPages = 10) {
      ...
      appendItems(
        await fetchChatGPTRootConversations(
          "",
          token,
          false,
          requested,
      ...
      for (const workspaceId of workspaceIds) {
        ...
        try {
          ...
          console.warn(
            "AI-Chat-Helper: 获取 ChatGPT 空间会话失败",
            workspaceId,
            e,
          );
        }
        ...
        try {
          ...
          for (const project of projects) {
            if (merged.length >= requested) break;
            appendItems(
              await fetchChatGPTProjectConversations(
                workspaceId,
                token,
                project,
                requested - merged.length,
          ...
          console.warn(
            "AI-Chat-Helper: 获取 ChatGPT 项目会话失败",
            workspaceId,
            e,
          );
        }
    ...
    async function fetchChatGPTConversationById(
      conversationId,
      workspaceId = "",
    ) {
      const token = await getChatGPTAccessToken();
      ...
      const resp = await chatGPTPageFetch(
    ...
    async function exportChatGPTBatchConversations(conversations, format) {
      await exportBatchConversationsAsZip(
        "ChatGPT",
        conversations,
        format,
        "ChatGPT",
    ...
    function isChatGPTToolCallJsonLine(line) {
      ...
      if (!text || text[0] !== "{" || text[text.length - 1] !== "}")
        return false;
      return /"(?:search_query|image_query|open|click|find|screenshot|finance|weather|sports|time|response_length)"\s*:/.test(
        text,
      );
    }
    ...
    function restoreChatGPTImplicitLists(text) {
      ...
      const listIntroPattern =
        /(?:分析|包括|包含|如下|参数|流程|步骤|指标|例如|分为|提取|判断|寻找|识别|输出|输入|类型|关系|结构|模块|功能|能力|字段|类别|清单)[：:]\s*$/;
    ...
    function isExportableChatGPTMessage(msg) {
      ...
      if (author !== "user" && author !== "assistant" && author !== "tool")
        return false;
      ...
      const contentType = String(
        msg.content?.content_type || msg.content?.type || "",
      )
        .trim()
        .toLowerCase();
      if (
    ...
    function buildChatGPTReferenceReplacement(reference, fallbackLabel) {
      const type = String(
        reference?.type || reference?.metadata?.type || "",
      ).toLowerCase();
    ...
    function applyChatGPTMessageReferences(text, msg) {
        ...
        if (exactItems.length) {
          ...
          fallbackItems.forEach((item) => {
            ...
            if (
              item.matchedText &&
              segment &&
              segment !== item.matchedText &&
              !segment.includes("filecite") &&
              !segment.includes("")
            )
              return;
            output =
      ...
      const citations = Array.isArray(msg?.metadata?.citations)
      ...
      return output
        .replace(
          /\uE200filecite(?:\uE202turn\d+file\d+)+\uE201/gi,
          "[文件引用]",
        )
        .replace(
          /\uE200cite(?:\uE202turn\d+(?:search|view)\d+)+\uE201/gi,
          "[引用]",
        )
    ...
    function buildArchivedMessageSnapshot(snapshot, urlMap) {
      const next = {
        ...
        text: replaceTextByUrlMap(
          snapshot?.text == null ? "" : String(snapshot.text),
          urlMap,
        ),
        parts: Array.isArray(snapshot?.parts)
          ? snapshot.parts.map((part) => replaceImageUrlsInPart(part, urlMap))
          : [],
      };
      next.__rawText = replaceTextByUrlMap(snapshot?.__rawText || "", urlMap);
      if (isClaude && next.parts.length) {
        next.__displayText =
          buildClaudeTextFromParts(next.parts, "archive") ||
          replaceTextByUrlMap(snapshot?.__displayText || "", urlMap);
        next.__markdownText =
          buildClaudeTextFromParts(next.parts, "markdown") ||
          replaceTextByUrlMap(snapshot?.__markdownText || "", urlMap);
      } else {
    ...
    function buildArchivedMessageSnapshotWithFiles(snapshot, urlMap, fileMap) {
      ...
      if (isClaude && next.parts.length) {
        next.__displayText =
          buildClaudeTextFromParts(next.parts, "archive") || next.__displayText;
        next.__markdownText =
          buildClaudeTextFromParts(next.parts, "markdown") ||
          next.__markdownText;
      }
    ...
    function collectExportInlineFiles(messages) {
      ...
      (Array.isArray(messages) ? messages : []).forEach((msg) => {
        ...
        parts.forEach((part) => {
          ...
          const content =
            typeof part.inlineContent === "string" ? part.inlineContent : "";
    ...
    async function buildImageArchiveBundle(messages) {
      ...
      inlineFiles.forEach((file, index) => {
        const baseName =
          String(file.fileName || "").replace(/\.[a-z0-9]+$/i, "") ||
          `交互演示${index + 1}`;
    ...
    function buildBatchConversationMarkdown(
      ...
      const body = (
        Array.isArray(preparedMessages)
          ? preparedMessages
    ...
    function renderClaudeCachedAttachmentsToStaticHtml(text) {
      return extractClaudeHtmlAttachmentNamesFromText(text)
        .map((name, idx) => {
    ...
    function renderClaudePartsToStaticHtml(parts, fallbackText) {
      ...
      if (!list.length) {
        const base =
          chatMarkdownToHtml(String(fallbackText || "")) ||
          escapeHtml(String(fallbackText || "")).replace(/\n/g, "<br>");
        const cachedPreview =
          renderClaudeCachedAttachmentsToStaticHtml(fallbackText);
      ...
      const renderInlineFile = (part, rawName) => {
        ...
        if (mediaParts.length)
          return renderClaudePartsToStaticHtml(mediaParts, "");
        if (isClaudeCadHoleWidgetHtml(inlineContent, rawName)) {
          return renderClaudePartsToStaticHtml(
            [buildClaudeWidgetFallbackSvgPart(inlineContent, rawName)],
            "",
          );
        }
        const parsed = parseClaudeWidgetHtmlSource(inlineContent);
      ...
      list.forEach((part) => {
        ...
        if (type === "text" || type === "tool_use" || type === "tool_result") {
          ...
          if (t)
            html.push(
              chatMarkdownToHtml(t) || escapeHtml(t).replace(/\n/g, "<br>"),
            );
          return;
        }
        if (type === "image") {
          ...
          const name = escapeHtml(
            String(part.fileName || "").trim() || `图片${imageSerial}`,
          );
        ...
        if (type === "svg") {
          imageSerial += 1;
          const name = escapeHtml(
            String(part.fileName || "").trim() || `图像${imageSerial}`,
          );
      ...
      const base =
        ...
        escapeHtml(String(fallbackText || "")).replace(/\n/g, "<br>");
      if (
        !/claude-html-widget-block|claude-image-block|claude-inline-svg|<svg\b|<img\b/i.test(
          base,
        )
      ) {
        const cachedPreview =
    ...
    async function exportBatchConversationsAsZip(
      platform,
      conversations,
      format,
      assistantLabel,
    ...
    function normalizeMarkdownForPreview(mdText) {
      ...
      return rawText
        ...
        .split("\n")
        .map((line) =>
          repairSplitMarkdownMarkers(String(line || "").replace(/[ \t]+$/g, ""))
            .replace(/^\s*(?:-\s+--+|--+)\s*$/g, "---")
            .replace(/^(\s*#{1,6})(?=\S)/, "$1 ")
            .replace(/^(\s*[-*+])(?=\S)/, "$1 ")
            .replace(/^(\s*\d+[.)])(?=\S)/, "$1 "),
        ...
        typeof markdownit === "function"
          ...
          : typeof window !== "undefined" &&
            ...
            : typeof unsafeWindow !== "undefined" &&
                typeof unsafeWindow.markdownit === "function"
    ...
    function chatMarkdownToHtml(mdText) {
      ...
      const libraryHtml = isChatGPT
        ? renderMarkdownWithLibrary(normalizedMd)
        : "";
    ...
    function partToMarkdown(part) {
      ...
      if (
        typeof part.recipient === "string" &&
        /^(?:browser|python|web|search|image|finance|weather|sports|time)\b/i.test(
          part.recipient.trim(),
        )
      ) {
        return "";
    ...
    function extractChatGPTMessageText(msg) {
      ...
      if (typeof content === "string")
        return cleanChatGPTText(applyChatGPTMessageReferences(content, msg));
      ...
      return cleanChatGPTText(
        applyChatGPTMessageReferences(
          String(raw).replace(/\n{3,}/g, "\n\n"),
          msg,
        ),
      );
    ...
    function extractChatGPTMessagesFromMapping(convData) {
      ...
      const pushMessageFromNode = (node) => {
        ...
        const attachmentText =
          author === "user" ? extractChatGPTUserAttachmentText(msg) : "";
        const toolSupplementText =
          author === "tool" ? extractChatGPTToolSupplementText(msg) : "";
        const mergedText = [toolSupplementText, text, attachmentText]
          .filter(Boolean)
          .join("\n\n");
    ...
    const QWEN_DEFAULT_MSG_LIST_URL =
      "https://chat2-api.qianwen.com/api/v1/session/msg/list?return_response_messages=true&biz_id=ai_qwen&event_filter=all&page_size=50&chat_client=h5&device=pc&fr=pc&pr=qwen&la=zh-CN&tz=Asia%2FShanghai";
    const QWEN_DEFAULT_PAGE_LIST_URL =
    ...
    function hasUsableQwenRequestTemplate() {
      ...
      try {
        const u = new URL(qwenCapturedTemplate.url, window.location.origin);
        const ut =
          (u.searchParams.get("ut") || "").trim() ||
          String(getQwenUtFromPage() || "").trim();
        if (!ut) return false;
    ...
    async function waitForQwenTemplateOrPending(timeoutMs = 6000) {
      ...
      while (Date.now() - startedAt < timeoutMs) {
        if (
          qwenPendingApiPayloads.length > 0 ||
          hasUsableQwenRequestTemplate()
        ) {
          return true;
        }
      ...
      return (
        qwenPendingApiPayloads.length > 0 || hasUsableQwenRequestTemplate()
      );
    }
    ...
    function ensureQwenRequestUrl(rawUrl, sessionId, extraParams = null) {
      const base =
        rawUrl || qwenCapturedTemplate?.url || QWEN_DEFAULT_MSG_LIST_URL;
      const u = new URL(base, window.location.origin);
      const isHistoryPage = Boolean(
        extraParams && Object.prototype.hasOwnProperty.call(extraParams, "pos"),
      );
    ...
    function extractQwenRecentConversations(respJson) {
      ...
      rawList.forEach((item, idx) => {
        ...
        const modified = normalizeQwenBatchTimestamp(
          findAny(item, [
            "modifiedTime",
            "modified_time",
            "updated_at",
        ...
        const created = normalizeQwenBatchTimestamp(
          findAny(item, [
            "createdTime",
            "created_time",
            "created_at",
    ...
    async function fetchQwenRecentConversations(limit = 50, maxPages = 5) {
      ...
      while (page < maxPages && merged.length < totalLimit) {
        ...
        if (!resp.ok) {
          throw new Error(
            `会话列表请求失败: HTTP ${resp.status} ${rawText.slice(0, 300)}`,
          );
        }
        if (!json) {
        ...
        if (!nextToken || !pageItems.length || merged.length >= totalLimit)
          break;
      }

      return {
        conversations: merged.sort(
    ...
    async function fetchQwenAllConversationMessages(
      sessionId,
      maxPages = 20,
      onProgress = () => {},
    ) {
      ...
      let currentUrl = ensureQwenRequestUrl(baseUrl, sessionId, {
        page_size: 50,
      });
      let lastPos = "";
      let page = 0;
      ...
      while (currentUrl && page < maxPages) {
        ...
        if (!resp.ok) {
          throw new Error(
            `消息请求失败: HTTP ${resp.status} ${rawText.slice(0, 300)}`,
          );
        }
        if (!json) {
        ...
        lastPos = nextPos;
        currentUrl = ensureQwenRequestUrl(baseUrl, sessionId, {
          page_size: 50,
          pos: nextPos,
        });
      }
      ...
      return {
        messages: all.sort((a, b) => (a.order || 0) - (b.order || 0)),
        pages: page,
      };
    }
    ...
    function getQwenNextPagePos(respJson) {
      ...
      return candidates.reduce(
        (min, cur) => (BigInt(cur) < BigInt(min) ? cur : min),
        candidates[0],
      );
    }
    ...
    function collectQwenAttachmentNames(value, out, depth = 0) {
      ...
      const resourceInfos = Array.isArray(value?.resource_infos)
    ...
    function parseQwenMessagesFromResponse(respJson) {
      ...
      arr.forEach((item, idx) => {
        ...
        const mergedUserText = Array.from(
          new Set(
    ...
    function captureQwenTemplateFromResponse(
      respJson,
      requestUrl,
      requestHeaders = null,
    ) {
      ...
      let finalUrl =
        requestUrl || qwenCapturedTemplate?.url || QWEN_DEFAULT_MSG_LIST_URL;
      try {
    ...
    installQwenCaptureHooks = function () {
      ...
      XMLHttpRequest.prototype.send = function (body) {
        try {
          if (
            this.__aiNodesQwenUrl &&
            isQwenMsgListUrl(this.__aiNodesQwenUrl)
          ) {
            captureQwenTemplate(
              this.__aiNodesQwenUrl,
            ...
            this.addEventListener(
              ...
              () => {
                try {
                  ...
                  const json = safeParseJson(rawText);
                  if (!json) return;
                  captureQwenTemplateFromResponse(
                    json,
                    this.__aiNodesQwenUrl,
                    this.__aiNodesQwenHeaders,
                  );
    ...
    async function fetchQwenUserNodesByTemplate() {
      ...
      const baseUrl = qwenCapturedTemplate?.url || QWEN_DEFAULT_MSG_LIST_URL;
      let requestMethod = normalizeQwenHttpMethod(
        qwenCapturedTemplate?.method,
        "GET",
      );
      ...
      try {
        ...
        while (page < maxPages) {
          page += 1;
          const currentUrl = ensureQwenRequestUrl(
            baseUrl,
            sessionId,
            pos ? { pos } : null,
          );
          ...
          for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            qwenNodeLog("node-api:method-fallback", {
              page,
              from: requestMethod,
              to: fallback,
              status: resp.status,
          ...
          if (!resp || !resp.ok) {
            ...
            qwenNodeLog("node-api:request-failed", {
              page,
        ...
        qwenSuppressCapturedPayloads = Math.max(
          0,
          qwenSuppressCapturedPayloads - 1,
        );
      }
    ...
    async function fetchQwenMessagesByTemplate() {
      ...
      try {
        ...
        while (currentUrl && page < 20) {
          ...
          if (!resp.ok) {
            console.warn(
              `AI-Chat-Helper: 千问会话 API 请求失败 (${resp.status})`,
              rawText.slice(0, 400),
            );
            break;
          }
    ...
    function maybeHydrateQwenHistory(triggerSource = "unknown") {
      ...
      if (!sessionId) return;
      const cacheSig = qwenVirtualNodesCache
        .map((m) => String(m.id || ""))
        .filter(Boolean)
        .join("|");
      if (
        qwenLastHydratedSessionId === sessionId &&
        qwenLastHydrationSignature === cacheSig &&
        cacheSig
      ) {
        return;
      }
    ...
    getQwenMessagesByApi = async function () {
      ...
      if (!hasUsableQwenRequestTemplate()) {
        qwenNodeLog("api:direct-fetch", {
          ...
          headerKeys: Object.keys(qwenCapturedTemplate?.headers || {}).slice(
            0,
            12,
          ),
        });
    ...
    installDoubaoCaptureHooks = function () {
      ...
      XMLHttpRequest.prototype.send = function (body) {
        try {
          ...
          if (
            this.__aiNodesDoubaoMethod === "POST" &&
    ...
    function buildDoubaoSingleChainUrl() {
      if (doubaoCapturedTemplate?.url)
        return ensureDoubaoSingleChainUrl(doubaoCapturedTemplate.url);
    ...
    function buildDoubaoRequest(convId) {
      ...
      const headers = sanitizeDoubaoHeaders(
        doubaoCapturedTemplate?.headers || {},
      );

      const templateBody = doubaoCapturedTemplate?.bodyText
    ...
    function extractDoubaoTextFromBlocks(blocks) {
      ...
      const formatDoubaoRequirementClarifyBlock = (reqBlock) => {
        ...
        requirements.forEach((group) => {
          const items = Array.isArray(group?.requirement_items)
      ...
      const walk = (node) => {
        ...
        if (
          referenceBlock &&
          referenceBlock.text &&
          typeof referenceBlock.text.text === "string"
        ) {
    ...
    function extractDoubaoRecentConversations(respJson) {
      ...
      const pushConv = (item) => {
        if (!item || typeof item !== "object") return;
        const id = String(
          item.conversation_id || item.conv_id || item.id || item.chat_id || "",
        ).trim();
        if (!id || seen.has(id)) return;
    ...
    function resolveDoubaoNextConvCursor(payload, pageConversations) {
      ...
      for (let i = arr.length - 1; i >= 0; i--) {
        const raw = arr[i]?.raw;
        const fromRaw = findDoubaoNestedVersion(
          raw,
          [
            "next_conv_version",
            "nextConvVersion",
    ...
    async function fetchDoubaoRecentConversations(customUrl = "", limit = 20) {
      ...
      const recentConvTemplateBody = safeParseJson(
        doubaoCapturedRecentConvBodyText || "",
      );
          ...
          while (keepPaging && merged.length < safeLimit && page < 12) {
            ...
            const buildMainBody = (mode) => {
              const body =
                ...
                typeof recentConvTemplateBody === "object"
              ...
              if (
                ...
                typeof body.uplink_body.pull_recent_conv_chain_uplink_body !==
                  "object"
              ) {
                body.uplink_body.pull_recent_conv_chain_uplink_body = {};
              }
              ...
              pullBody.conv_version =
                normalizeDoubaoConvVersionValue(convVersionCursor);
              if (pullBody.api_version == null) pullBody.api_version = 1;
              pullBody.direction = isContinuationPage
                ? 1
                : pullBody.direction == null
                  ? 3
                  : pullBody.direction;
              if (pullBody.message_count_per_conv == null)
              ...
              if (isContinuationPage) {
                ...
                if (pullBody.option.need_coco_conversation == null)
                  pullBody.option.need_coco_conversation = true;
                if (pullBody.option.need_coco_bot == null)
                  pullBody.option.need_coco_bot = true;
                if (pullBody.option.need_pc_pin_chain == null)
                  pullBody.option.need_pc_pin_chain = true;
                if (pullBody.option.pc_pin_query_type == null)
            ...
            const payload =
              json?.downlink_body?.pull_recent_conv_chain_downlink_body ||
              json?.downlink_body?.pull_recent_conv_downlink_body ||
              {};

            const hasMore =
              Boolean(payload?.has_more || payload?.hasMore) ||
              pageConversations.length >= requestLimit;

            const nextConvVersion = resolveDoubaoNextConvCursor(
              payload,
              pageConversations,
            );
            ...
            if (!hasMore || added === 0) {
              ...
              const currentCursorText = String(
                convVersionCursor == null ? "" : convVersionCursor,
              ).trim();
              if (
                !normalized ||
    ...
    function extractDoubaoArtifactText(json) {
      ...
      const walk = (node) => {
        ...
        const fileName = String(
          node?.name || node?.file_name || node?.filename || node?.path || "",
        ).trim();
        ...
        if (Array.isArray(node?.files)) {
          node.files.forEach((f) => {
            const fn = String(
              f?.name || f?.file_name || f?.filename || f?.path || "",
            ).trim();
    ...
    async function fetchDoubaoArtifactContent(
      ...
      try {
        ...
        const finalText =
          extracted ||
          (typeof raw === "string" && raw.length < 50000 ? raw.trim() : "");
    ...
    function getDisplayTextForExport(rawText) {
      ...
      const normalizeLines = (inputText) => {
        ...
        lines.forEach((line) => {
          ...
          if (
            /^https?:\/\//i.test(line) &&
            /(byteimg|tos-cn-|flow-sign|flow-imagex-sign)/i.test(line)
          )
            return;
    ...
    function normalizeChatGPTImageAssetUrl(rawUrl) {
      ...
      if (/^sediment:\/\//i.test(raw)) {
        const assetId = raw.replace(/^sediment:\/\//i, "").trim();
        return assetId
          ? `https://chatgpt.com/backend-api/files/${encodeURIComponent(assetId)}/content`
          : "";
      }
      return resolveClaudeAssetUrl(raw);
    }
    ...
    function getChatGPTImageAssetUrlFromMsg(msg) {
      ...
      for (const part of parts) {
        ...
        const metadataUrl =
          part.metadata?.asset_pointer_link ||
    ...
    function renderChatGPTImagePreviewHtml(msg, fallbackText) {
      ...
      if (url) {
        parts.push(
          `<div class="m-preview-media"><img src="${escapeHtmlAttr(url)}" alt="${escapeHtmlAttr(title || "图片")}" style="max-width:100%;max-height:320px;border-radius:12px;display:block;margin-bottom:8px;"><div class="m-preview-caption">${escapeHtml(title || fallbackText || "图片生成结果")}</div></div>`,
        );
      }
      if (!parts.length && fallbackText) {
        parts.push(renderMessageMarkdownToPreviewHtml(fallbackText));
      }
      return parts.join("");
    }
    ...
    function renderClaudePartsToPreviewHtml(parts, fallbackText) {
      ...
      list.forEach((part) => {
        ...
        if (type === "image") {
          ...
          if (url) {
            html.push(
              `<div class="m-preview-media"><img src="${url}" alt="${name}" style="max-width:100%;max-height:220px;border-radius:10px;display:block;margin-bottom:8px;"><div class="m-preview-caption">图片${imageSerial} · ${name}</div></div>`,
            );
          } else {
        ...
        if (type === "svg") {
          ...
          const svgMarkup = String(part.svg || "").trim();
          const svgPreviewUrl = escapeHtmlAttr(
            getClaudeInlineSvgPreviewUrl(svgMarkup) ||
              buildClaudeSvgDataUrl(svgMarkup),
          );
          ...
          if (svgMarkup) {
            html.push(
              `<div class="m-preview-media"><div class="claude-inline-svg"><img src="${svgPreviewUrl}" alt="${name}" loading="lazy" onerror="this.closest('.claude-inline-svg').setAttribute('data-ai-svg-error','1');this.remove();"></div><div class="m-preview-caption">图像${imageSerial} · ${name}${ratioText}</div></div>`,
            );
          } else {
        ...
        if (type === "file") {
          fileSerial += 1;
          const name = escapeHtml(
            String(part.fileName || "").trim() || `附件${fileSerial}`,
          );
          const url = escapeHtml(resolveClaudeAssetUrl(part.url || ""));
          const inlineContent = removeClaudePromptSuggestionsFromHtml(
            part.inlineContent || "",
          );
          const isHtmlWidget =
            inlineContent &&
            /(?:<script\b|onclick=|oninput=|id=["']vis-container["']|id=["']hole-svg["'])/i.test(
              inlineContent,
            );
          if (isHtmlWidget) {
            const widgetMediaParts =
              extractClaudeRenderablePartsFromHtmlFragment(inlineContent);
            if (widgetMediaParts.length) {
              html.push(
                widgetMediaParts
                  .map((mp) => renderClaudePartsToPreviewHtml([mp], ""))
                  .join(""),
              );
              return;
            }
            if (
              isClaudeCadHoleWidgetHtml(
                inlineContent,
                String(part.fileName || "").trim() || name,
              )
            ) {
              html.push(
                renderClaudePartsToPreviewHtml(
                  [
                    buildClaudeWidgetFallbackSvgPart(
                      inlineContent,
                      String(part.fileName || "").trim() || name,
                    ),
                  ],
            ...
            const staticHtmlPreview = buildClaudeWidgetRuntimeBlock(
              inlineContent,
              String(part.fileName || "").trim() || name,
            );
            if (staticHtmlPreview) {
              html.push(
                `<div class="m-preview-media m-preview-widget">${staticHtmlPreview}<div class="m-preview-caption">附件${fileSerial} · ${name}</div></div>`,
              );
              return;
            }
            html.push(
            ...
            const mediaParts = inlineContent
              ? extractClaudeRenderablePartsFromHtmlFragment(inlineContent)
              : [];
            if (mediaParts.length) {
              html.push(
    ...
    function buildMessagePreviewContent(msg) {
      ...
      const parts = Array.isArray(msg?.parts) ? msg.parts : [];
      if (isChatGPT && msg?.role === "assistant" && Array.isArray(msg?.sourceParts)) {
      ...
      if (
        isClaude &&
        !/claude-widget-runtime|m-preview-widget|claude-inline-svg|<svg\b|<img\b/i.test(
          html,
        )
      ) {
        const cachedPreview = renderClaudeCachedAttachmentPreviewFromText(
    ...
    async function parseDoubaoSingleChainMessages(
      ...
      for (const m of rawMessages) {
        ...
        const blockText = Array.isArray(m?.content_block)
          ...
          : "";
        const artifactText = await extractDoubaoArtifactTextFromContentBlocks(
          m?.content_block,
          requestHeaders,
        );
        const contentText = parseDoubaoContentPayload(m?.content);
        const ttsText =
          typeof m?.tts_content === "string" ? m.tts_content.trim() : "";
    ...
    async function fetchDoubaoConversationMessagesByApi(
      ...
      const bodyObj =
        pullBody && typeof pullBody === "object"
          ? pullBody
          : {
              cmd: 3100,
              uplink_body: {
      ...
      if (
        ...
        typeof bodyObj.uplink_body.pull_singe_chain_uplink_body !== "object"
      ) {
        bodyObj.uplink_body.pull_singe_chain_uplink_body = {};
      }
      ...
      const hasValidNextIndex =
        Number.isFinite(nextIndexNum) && nextIndexNum > 0;
      ...
      while (!done && page < maxPages) {
        ...
        if (parsed.length > 0) {
          const firstId = String(
            parsed[0]?.sourceMessageId || parsed[0]?.id || "",
          ).trim();
          const lastId = String(
            parsed[parsed.length - 1]?.sourceMessageId ||
    ...
    async function fetchClaudeRecentConversations(limit = 30) {
      ...
      out.sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
      return {
        conversations: out.slice(0, safeLimit),
        requested: safeLimit,
        obtained: out.length,
      };
    }
    ...
    async function fetchClaudeConversationMessages(conversationId) {
      ...
      if (!orgId) throw new Error("未从 cookie 中获取到 lastActiveOrg");
      const u = new URL(
        `https://claude.ai/api/organizations/${encodeURIComponent(orgId)}/chat_conversations/${encodeURIComponent(conversationId)}`,
      );
      u.searchParams.set("tree", "True");
      u.searchParams.set("rendering_mode", "messages");
      u.searchParams.set("render_all_tools", "true");
      ...
      if (!resp.ok)
        throw new Error(`chat_conversations 请求失败: ${resp.status}`);
      const raw = await resp.text();
      let json = null;
      try {
        json = JSON.parse(raw);
      ...
      return {
        id:
          String(json?.uuid || conversationId || "").trim() ||
    ...
    function renderBatchConversationList(
      listEl,
      conversations,
      buildMetaTags,
      emptyText,
      ...
      listEl.innerHTML = conversations
        .map((conversation, idx) => {
          const metaTags = (
            typeof buildMetaTags === "function"
              ? buildMetaTags(conversation)
              : []
          )
            .filter(Boolean)
            .map(
            ...
            .join("");
          const rowKey = escapeHtml(
            String(conversation.key || conversation.id || idx),
          );
          return `
    ...
    function buildUnifiedBatchMetaTags(conversation, options = {}) {
      ...
      const messageCountRaw =
        conv.badgeCount != null ? conv.badgeCount : conv.messageCount;
      const messageCount = Number(messageCountRaw);
      const messageCountText = Number.isFinite(messageCount)
        ? String(messageCount)
        : "-";
    ...
    async function hydrateConversationMessageCounts(
      conversations,
      options = {},
    ) {
      const list = Array.isArray(conversations) ? conversations : [];
      const getCount =
        typeof options.getCount === "function" ? options.getCount : null;
      const onProgress =
        typeof options.onProgress === "function" ? options.onProgress : null;
      const isCancelled =
        typeof options.isCancelled === "function" ? options.isCancelled : null;
      const shouldHydrate =
        typeof options.shouldHydrate === "function"
      ...
      const concurrency = Math.max(
        1,
        Math.min(6, Number(options.concurrency) || 2),
      );
      if (!getCount || !list.length)
        return { total: 0, completed: 0, resolved: 0 };
    ...
    function setupUnifiedModalLifecycle(overlay, modal, options = {}) {
      ...
      const onKeydown = (e) => {
        if (e.key !== "Escape") return;
        if (activeModalCloseStack[activeModalCloseStack.length - 1] !== close)
          return;
        e.preventDefault();
        close();
      };
    ...
    async function openMessagePreviewExportModal(options = {}) {
      ...
      const loadingTitle = String(
        options.loadingTitle || "正在加载对话内容...",
      );
      const loadingHint = String(
        options.loadingHint || "请稍候，导出列表即将就绪",
      );
      const emptyText = String(options.emptyText || "未检测到可导出的内容");
      const headerMetaText = String(options.headerMetaText || "").trim();
      const loader =
        typeof options.loader === "function" ? options.loader : null;
      const initialSelectedIndices = Array.isArray(
      ...
      persistSelection = () => {
        if (onSelectionChange)
          onSelectionChange(getSelectedIndices(), allMsgs.slice());
      };
      ...
      const getDisplayLabel = (msg) => {
        ...
        if (isDeepSeek) {
          ...
          if (
            msg.isSearch ||
            String(msg.fragmentType || "").toUpperCase() === "SEARCH"
          )
            return "DeepSeek 智能搜索";
          if (String(msg.fragmentType || "").toUpperCase() === "RESPONSE")
            return "DeepSeek AI回答";
        }
        return AI_NAME;
      };
      ...
      allMsgs.forEach((m, i) => {
        ...
        if (
          rowTextEl.querySelector(
            ".m-preview-widget, .claude-widget-runtime, .claude-inline-svg, svg, img",
          )
        ) {
      ...
      function showFullText(content, title) {
        ...
        const contentHtml =
          typeof content === "string" ? "" : String(content?.html || "").trim();
      ...
      modal.addEventListener("click", (e) => {
        ...
        if (exportMenu.contains(target) || exportMenuTrigger.contains(target))
          return;
        hideExportMenu();
      });
      ...
      if (isDeepSeek && modal.querySelector("#m-no-thought")) {
        modal.querySelector("#m-no-thought").onclick = () => {
          allMsgs.forEach((m, i) => {
            const ck = modal.querySelector(`.m-row-ck[data-i="${i}"]`);
            const rowText = modal
              .querySelector(`.m-row-ck[data-i="${i}"]`)
              ?.closest(".m-item-row")
              ?.querySelector(".m-row-text");
            if (m.isThought) {
            ...
            if (m.hasThought && m.textWithoutThought) {
              ...
              if (rowText) {
                ...
                rowText.classList.toggle(
                  "has-widget-preview",
                  Boolean(
                    rowText.querySelector(
                      ".m-preview-widget, .claude-widget-runtime, .claude-inline-svg, svg, img",
                    ),
                  ),
                );
    ...
    function openBatchConversationPreviewModal(
      platformLabel,
      conversationTitle,
      loader,
    ) {
    ...
    function bindBatchConversationListInteractions(
      listEl,
      lookupConversation,
      onView,
      onSelectionChange,
      ...
      listEl.addEventListener("click", async (e) => {
        ...
        if (viewHit && listEl.contains(viewHit)) {
          ...
          const conversation =
            typeof lookupConversation === "function"
              ? lookupConversation(key)
              : null;
    ...
    function openChatGPTBatchExportModal() {
      ...
      const exportMenuTrigger = modal.querySelector(
        "#gpt-batch-export-menu-trigger",
      );
      ...
      const openConversationPreview = async (conv) => {
        const previewLoader = async () => {
          const convData = await fetchChatGPTConversationById(
            conv.id,
            conv.workspaceId,
          );
          ...
          return {
            title:
              String(convData?.title || conv.title || "").trim() ||
              `会话 ${conv.id}`,
        ...
        previewLoader.onSelectionChange = (indices) =>
          storeSelection(conv, indices);
        openBatchConversationPreviewModal(
          "ChatGPT",
          conv.title || conv.id,
          previewLoader,
        );
      ...
      const runBatchExport = async (format) => {
        ...
        for (let i = 0; i < selected.length; i += 1) {
          ...
          try {
            ...
            const messages = extractChatGPTMessagesFromMapping(convData).map(
              (m) => ({
                role: m.role,
                text: String(m.text || ""),
              }),
            );
            const pickedMessages = applyMessageSelection(
              messages,
            ...
            failCount += 1;
            console.warn(
              "AI-Chat-Helper: ChatGPT 批量导出会话失败",
              conv?.id,
              e,
            );
          }
        ...
        alert(
          `批量导出完成：成功 ${out.length} 个会话${failCount ? `，失败 ${failCount} 个` : ""}。已按会话标题分别打包为 ZIP。${format === "html" ? "HTML 文件可直接打开浏览。" : ""}`,
        );
      };
    ...
    function openClaudeBatchExportModal() {
        ...
        for (let i = 0; i < selected.length; i += 1) {
          ...
          try {
            ...
            out.push({
              conversationId: conv.id,
              title:
                String(info?.title || conv.title || "").trim() ||
                `会话 ${conv.id}`,
              updatedAt: conv.updatedAt || 0,
              updatedAtText: conv.updatedAtText || "",
              createdAt: conv.createdAt || 0,
            ...
            failCount += 1;
            console.warn(
              "AI-Chat-Helper: Claude 批量导出会话失败",
              conv?.id,
              e,
            );
          }
      ...
      bindBatchConversationListInteractions(
        listEl,
        (key) =>
          recentConversations.find((c) => String(c.id) === String(key)) || null,
        openConversationPreview,
        updateSelectToggleButton,
      );
    ...
    function ensureDeepSeekPageListUrl(cursor) {
      const base =
        deepseekPageListTemplate?.url ||
        `https://chat.deepseek.com${DEEPSEEK_PAGE_LIST_PATH}?lte_cursor.pinned=false`;
      const u = new URL(base, window.location.origin);
      u.pathname = DEEPSEEK_PAGE_LIST_PATH;
      if (!u.searchParams.has("lte_cursor.pinned"))
        u.searchParams.set("lte_cursor.pinned", "false");
      if (cursor && typeof cursor === "object") {
        Object.entries(cursor).forEach(([k, v]) => {
          if (v == null || v === "") return;
          const key = String(k).startsWith("lte_cursor.")
            ? String(k)
            : `lte_cursor.${String(k)}`;
    ...
    function extractDeepSeekBatchConversationsFromResponse(respJson) {
      ...
      rawList.forEach((item, idx) => {
        ...
        const updated = normalizeDeepSeekBatchTimestamp(
          findAnyDeepSeek(item, [
            "updated_at",
            "update_time",
            "modified_at",
        ...
        const created = normalizeDeepSeekBatchTimestamp(
          findAnyDeepSeek(item, [
            "inserted_at",
            "created_at",
            "create_time",
        ...
        const messageCountRaw = findAnyDeepSeek(item, [
          "message_count",
          "msg_count",
          "badge_count",
          "messageCount",
    ...
    async function fetchDeepSeekConversationMessages(sessionId) {
      ...
      const messages = aggregateDeepSeekMessagesForExport(
        parseDeepSeekMessagesFromResponse(json),
      ).map((msg) => ({
        role: msg.role,
        ...
        try {
          ...
          if (missingTotal > 0) {
            ...
            hydrateConversationMessageCounts(recentConversations, {
              concurrency: 2,
              isCancelled: () =>
                runSeq !== countHydrationSeq || !modal.isConnected,
              shouldHydrate: (conv) => {
              ...
              getCount: async (conv) => {
                ...
                const apiCreatedAtValue = Number(
                  apiSession?.insertedAtValue || 0,
                );
                const apiUpdatedAtValue = Number(
                  apiSession?.updatedAtValue || 0,
                );
                const apiCreatedAtText = String(
                  apiSession?.insertedAt || "",
                ).trim();
                const apiUpdatedAtText = String(
                  apiSession?.updatedAt || "",
                ).trim();

                if (Number.isFinite(apiCreatedAtValue) && apiCreatedAtValue > 0)
                  conv.createdAt = apiCreatedAtValue;
                if (apiCreatedAtText) conv.createdAtText = apiCreatedAtText;
                if (Number.isFinite(apiUpdatedAtValue) && apiUpdatedAtValue > 0)
                  conv.updatedAt = apiUpdatedAtValue;
                if (apiUpdatedAtText) conv.updatedAtText = apiUpdatedAtText;

                return Array.isArray(allRes?.messages)
                  ? allRes.messages.length
                  : null;
              },
        ...
        for (let i = 0; i < selected.length; i += 1) {
          ...
          try {
            ...
            out.push({
              ...
              title: conv.title,
              updatedAt:
                Number.isFinite(apiUpdatedAtValue) && apiUpdatedAtValue > 0
                  ? apiUpdatedAtValue
                  : conv.updatedAt || 0,
              updatedAtText: apiUpdatedAtText || conv.updatedAtText || "",
              createdAt:
                Number.isFinite(apiCreatedAtValue) && apiCreatedAtValue > 0
                  ? apiCreatedAtValue
                  : conv.createdAt || 0,
            ...
            failCount += 1;
            console.warn(
              "AI-Chat-Helper: DeepSeek 批量导出会话失败",
              conv?.id,
              e,
            );
          }
        ...
        try {
          ...
          if (missingTotal > 0) {
            ...
            hydrateConversationMessageCounts(recentConversations, {
              ...
              getCount: async (conv) => {
                const allRes = await fetchQwenAllConversationMessages(
                  conv.id,
                  20,
                  () => {},
                );
    ...
    async function getAllMessages() {
      ...
      if (isChatGPT) {
        ...
        if (apiMsgs.length) {
          ...
          console.warn(
            "AI-Chat-Helper: ChatGPT 导出已禁用 DOM 回退，当前仅支持 API 获取，避免虚拟列表导致内容缺失。",
          );
        ...
        if (apiMsgs.length) {
          ...
          console.warn(
            "AI-Chat-Helper: 千问导出已禁用 DOM 回退，当前仅支持 API 获取。",
          );
          return { messages: [], source };
        }
      } else if (isDoubao) {
        ...
        if (apiMsgs.length) {
          ...
          console.warn(
            "AI-Chat-Helper: 豆包导出已禁用 DOM 回退，当前仅支持 API 获取。",
          );
          return { messages: [], source };
        }
      } else if (isDeepSeek) {
        ...
        if (apiMsgs.length) {
          ...
          console.warn(
            "AI-Chat-Helper: DeepSeek 导出已禁用 DOM 回退，当前仅支持 API 获取。",
          );
          return { messages: [], source };
        }
      } else if (isClaude) {
        ...
        if (apiMsgs.length) {
          ...
          source =
            "API(/api/organizations/{org}/chat_conversations/{id})-FAILED";
          console.warn(
            "AI-Chat-Helper: Claude 导出已禁用 DOM 回退，当前仅支持 API 获取。",
          );
          return { messages: [], source };
        }
      }
        ...
        const shouldMerge =
          ...
          (!isDeepSeek ||
            (Boolean(last.isThought) === Boolean(msg.isThought) &&
              ...
              String(last.fragmentType || "") ===
                String(msg.fragmentType || "")));
    ...
    async function handleExport(data, format) {
      ...
      function renderInlineMarkdownForPdf(text) {
        ...
        tokens.forEach((t) => {
          ...
          if (t.type === "code") {
            ...
            out = out.replaceAll(
              escapedKey,
              `<span class="math-inline">\\(${escapeHtml(t.content)}\\)</span>`,
            );
          } else if (t.type === "math-display") {
            out = out.replaceAll(
              escapedKey,
              `<div class="math-display">\\[${escapeHtml(t.content)}\\]</div>`,
            );
          }
        });
        return out;
      ...
      function renderClaudePartsToHtmlForPdf(parts, fallbackText) {
        ...
        const renderInlineClaudeFileForPdf = (part, rawName) => {
          ...
          const mediaParts =
            extractClaudeRenderablePartsFromHtmlFragment(inlineContent);
          if (mediaParts.length)
            return renderClaudePartsToHtmlForPdf(mediaParts, "");
          if (isClaudeCadHoleWidgetHtml(inlineContent, rawName)) {
            return renderClaudePartsToHtmlForPdf(
              [buildClaudeWidgetFallbackSvgPart(inlineContent, rawName)],
              "",
            );
          }
          const parsed = parseClaudeWidgetHtmlSource(inlineContent);
          const bodyHtml = sanitizeClaudeWidgetBodyHtmlForPdf(
            parsed.bodyHtml || inlineContent,
          );
          if (!bodyHtml) return "";
        ...
        list.forEach((part) => {
          ...
          if (type === "image") {
            ...
            if (url) {
              html.push(
                `<figure class="claude-image-block"><img src="${url}" alt="${name}" loading="lazy"><figcaption>图片${imageSerial}${metaText ? ` · ${metaText}` : ""}</figcaption></figure>`,
              );
            } else {
              html.push(
                `<p>[图片${imageSerial}] ${name}${ratioText ? ` (${ratioText})` : ""}</p>`,
              );
            }
            return;
          }
          if (type === "svg") {
            ...
            if (svgMarkup) {
              html.push(
                `<figure class="claude-image-block claude-svg-block"><div class="claude-inline-svg">${svgMarkup}</div><figcaption>图像${imageSerial}${name ? ` · ${name}` : ""}${ratioText ? ` · ${ratioText}` : ""}</figcaption></figure>`,
              );
            } else {
              html.push(
                `<p>[图像${imageSerial}] ${name}${ratioText ? ` (${ratioText})` : ""}</p>`,
              );
            }
            return;
          }
      ...
      function renderMarkdownToHtmlForPdf(text) {
        ...
        const flushParagraph = () => {
          ...
          html.push(
            `<p>${paragraph.map((l) => renderInlineMarkdownForPdf(l)).join("<br>")}</p>`,
          );
          paragraph = [];
        };
        ...
        const flushList = () => {
          ...
          const items = listItems
            .map((item) => `<li>${renderInlineMarkdownForPdf(item)}</li>`)
            .join("");
        ...
        const flushQuote = () => {
          ...
          html.push(
            `<blockquote>${quoteLines.map((l) => renderInlineMarkdownForPdf(l)).join("<br>")}</blockquote>`,
          );
          quoteLines = [];
        };
        ...
        for (let i = 0; i < lines.length; i += 1) {
          ...
          if (displayMathMatch) {
            ...
            flushQuote();
            html.push(
              `<div class="math-display">\\[${escapeHtml(displayMathMatch[1])}\\]</div>`,
            );
            continue;
          }
          ...
          if (headingMatch) {
            ...
            html.push(
              `<h${level}>${renderInlineMarkdownForPdf(headingMatch[2])}</h${level}>`,
            );
            continue;
          }
          ...
          if (line.includes("|") && isTableSeparator(lines[i + 1])) {
            ...
            html.push(
              `<div class="table-wrap"><table class="pdf-table">${colgroup}${thead}${tbody}</table></div>`,
            );
      ...
      if (format === "txt") {
        ...
        for (let i = 0; i < exportData.length; i++) {
          ...
          if (exportData[i].role === "user") {
            while (
              i + 1 < exportData.length &&
              exportData[i + 1].role === "assistant"
            ) {
    ...
    const detectSearchEntryRole = (el) => {
      ...
      if (testId === "receive_message" || testId.includes("assistant"))
        return "assistant";
      const plugin = String(
      ...
      if (/(answer|assistant|bot|receive_message)/.test(cls))
        return "assistant";
      return "unknown";
    };
    ...
    const collectSearchEntriesFromDom = () => {
      ...
      if (isClaude) {
        ...
        Array.from(root.querySelectorAll('h2, [role="heading"]')).forEach(
          (h) => {
            if (!(h instanceof HTMLElement)) return;
            const ht = normalizeClaudeMatchText(h.innerText || "");
            if (!ht.includes("you said")) return;
    ...
    const normalizeSearchPlainText = (text) =>
      String(text || "")
        .replace(/\s+/g, " ")
        .trim()
        .toLowerCase();
    ...
    const buildSearchHitsByKeyword = (keyword) => {
      ...
      entries.forEach((entry, entryIdx) => {
        ...
        const nodeGroupIndex = getSearchEntryNodeGroupIndex(
          entry,
          entryIdx + 1,
        );
        let fromIndex = 0;
    ...
    const jumpToSearchHit = (hit, keyword) => {
      ...
      const highlightEl =
        getSearchHighlightContainer(hit.element) || hit.element;
      try {
      ...
      setTimeout(() => {
        const range = focusKeywordOccurrenceInElement(
          hit.element,
          keyword,
          Number(hit.hitIndexInEntry || 1),
        );
        if (range) {
    ...
    const renderGroupedSearchResultsContent = () => {
      ...
      const groupRows = currentSearchGroups
        .map((group) => {
          ...
          const hitIndexes = Array.isArray(group.hitIndexes)
            ? group.hitIndexes
            : [];
    ...
    const toggleSearchPopup = (e) => {
      ...
      const canUseLeft = leftSide >= margin;
      const canUseRight =
        rightSide + controlsWidth <= window.innerWidth - margin;
      const openDirection = canUseLeft
        ? "left"
        : canUseRight
          ? "right"
      ...
      const left = Math.max(
        margin,
        Math.min(rawLeft, window.innerWidth - controlsWidth - margin),
      );
      ...
      const computedControlsHeight =
        parseFloat(
          window.getComputedStyle(searchControlsPopup).height || "0",
        ) || 0;
      const roughControlsHeight = Math.round(
        searchControlsPopup.offsetHeight || computedControlsHeight || 32,
      );
      const controlsHeight = Math.max(32, roughControlsHeight);
      const canTopAlign =
        rect.top >= margin &&
        rect.top + controlsHeight <= window.innerHeight - margin;
      const canBottomAlign =
        rect.bottom - controlsHeight >= margin &&
      ...
      let controlsTop =
        verticalAlignMode === "top" ? rect.top : rect.bottom - controlsHeight;
      controlsTop = Math.max(
        margin,
        Math.min(controlsTop, window.innerHeight - controlsHeight - margin),
      );
      const hiddenTransform =
        openDirection === "left"
          ? "translateX(8px) scale(0.96)"
    ...
    function applyAutoCollapse() {
      ...
      const icon = sidebar.querySelector(
        'span[data-icon-type="qwpcicon-sidebarLeft"]',
      );
      const toggleBtn =
        (icon ? icon.closest("button") : null) ||
        sidebar.querySelector(
          'button[aria-label*="收起"], button[title*="收起"], button[class*="sidebar"]',
        ) ||
        document.querySelector(
          'button[aria-label*="收起侧边栏"], button[title*="收起侧边栏"], button[aria-label*="侧边栏"], button[title*="侧边栏"]',
        );
    ...
    function findDeepSeekHeaderAnchor() {
      ...
      const candidates = Array.from(document.querySelectorAll("svg path"))
        ...
        .map((p) =>
          p.closest(
            'div[role="button"], button, .ds-icon-button, .ds-icon-button--sizing-container',
          ),
        )
        .filter(Boolean)
      ...
      if (candidates.length) {
        ...
        candidates.forEach((el) => {
          ...
          if (
            cls.includes("ds-icon-button--xl") ||
            cls.includes("ds-icon-button--l")
          )
            score += 20;

          if (
            el.closest(
              '[class*="_0efe408"], [class*="_2be88ba"], [class*="a02af2e6"]',
            )
          )
            score += 30;
          ...
          if (
            el.closest(
              '[data-virtual-list-item-key], .ds-message, [class*="_77cdc67"], [class*="_3098d02"], [class*="_254829d"]',
            )
          )
            score -= 200;
      ...
      const topbar =
        document.querySelector(
          'div[class*="_2be88ba"] div[class*="_0efe408"]',
        ) ||
      ...
      const fallbacks = Array.from(
        topbar.querySelectorAll(
          'div[role="button"].ds-icon-button--sizing-container, .ds-icon-button.ds-icon-button--sizing-container',
        ),
      ).filter((el) => !el.classList.contains("ai-nodes-settings-btn"));
    ...
    function ensureDeepSeekPlacement() {
      ...
      const hostWidth = Math.max(
        32,
        Math.round(buttonHost.getBoundingClientRect().width || 32),
      );
      const gap = 8;
      const minLeft = 8;
      const maxLeft = Math.max(
        minLeft,
        Math.round(containerRect.width - hostWidth - 8),
      );

      let left = Math.round(anchorRect.right - containerRect.left + gap);
      if (left > maxLeft) {
        left = Math.round(
          anchorRect.left - containerRect.left - hostWidth - gap,
        );
      }
    ...
    function getQwenUploadRecordButton() {
      const candidates = Array.from(
        document.querySelectorAll(
          'button[aria-label="上传记录"], button[aria-label="实时记录"]',
        ),
      );
      if (!candidates.length) return null;
      const filtered = candidates.filter((btn) => {
        ...
        const rect = btn.getBoundingClientRect();
        return (
          rect.top >= 0 && rect.top < Math.max(220, window.innerHeight * 0.35)
        );
      });
    ...
    function getQwenUploadRecordAnchor() {
      ...
      const shell =
        btn.closest(
          '[class*="capsuleTransitionShell"], [class*="moreButtonMotionShell"]',
        ) ||
        btn.closest(
          '[class*="capsuleTransitionItem"], [class*="moreButtonMotionItem"]',
        ) ||
        btn;
